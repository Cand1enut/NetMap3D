...
function buildWall(w) {
  ...
  const h = w.h || WALL_H;
  const m = new THREE.Mesh(new THREE.BoxGeometry(len, h, WALL_T), makeWallMaterial(len, h, w.material));
...
function showWallProps(id) {
  ...
  matSel.onchange = () => {
    w.material = matSel.value;
    const m = wallMeshes.get(id);
    if (m) { wallMats.delete(m.material); m.material.dispose && m.material.dispose();
      m.material = makeWallMaterial(len, w.h || WALL_H, w.material); }
  };
