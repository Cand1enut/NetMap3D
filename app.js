...
function rackDressRoute(cable, a, b) {
  ...
  if (dy <= RACK_FACE_MAX_DY) {
    // Run across at the manager if there is one, otherwise midway between the
    // two ports so the horizontal leg sits in the gap rather than across either
    // faceplate. An empty list would mean "no dressing at all", so this always
    // returns its two points.
    const runY = hcmBetweenY(da.rackId, lan.y, lbn.y) ?? (lan.y + lbn.y) / 2;
    return [
      toWorld(new THREE.Vector3(lan.x, runY, z)),
      toWorld(new THREE.Vector3(lbn.x, runY, z))
    ];
  }
