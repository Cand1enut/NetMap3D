...
function iosPing(dev, target) {
  ...
  // A router pings from itself: reachable if it has a route and the target answers.
  const route = lookupRoute(dev, ip.int) || deviceInterfaces(dev).find(i => sameSubnet(i.ip, ip));
  const ok = !!owner && !!route;
