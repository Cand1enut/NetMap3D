...
const WAN_PACK = {
  ...
  internet: {
    label: 'Internet (ISP)', short: 'WAN', uh: 0, ports: 24, rows: 2, depth: 0,
  ...
  ont: {
    label: 'Fiber ONT (demarc)', short: 'ONT', uh: 0, ports: 9, rows: 1, depth: 0,
  ...
  modem: {
    label: 'Cable modem (demarc)', short: 'Modem', uh: 0, ports: 9, rows: 1, depth: 0,
...
// The scene is static between edits, so re-rendering the shadow map every frame
// is pure waste — at 800 devices it measured ~4 ms/frame of pointless work, and
// it scales with device count. The map is refreshed on demand instead:
// `markShadowsDirty()` after anything moves, is built, or is deleted.
...
// Real buildings are not all painted drywall. Each material has its own albedo
// and a height field turned into a normal map, so brick reads as brick with
// actual relief, not a flat photo. A wall carries `w.material`; drywall is the
// default so existing maps are unchanged. This is the geometry-and-material
// realism the owner is after (brick loft, exposed concrete, wood accent).
function brickCanvas(mode) {
  const S = 512, c = canvas2d(S), g = c.getContext('2d');
  const pxPerIn = S / WALL_TEX_IN;                 // 8 ft tile
  const bw = 8 * pxPerIn, bh = 2.25 * pxPerIn, mortar = 0.4 * pxPerIn;
  if (mode === 'albedo') { g.fillStyle = '#8f8a83'; g.fillRect(0, 0, S, S); }  // mortar
  else { g.fillStyle = '#3a3a3a'; g.fillRect(0, 0, S, S); }                    // mortar sits low
  let row = 0;
  for (let y = 0; y < S + bh; y += bh + mortar) {
    const off = (row % 2) ? -bw / 2 : 0;            // running bond
    for (let x = -bw; x < S + bw; x += bw + mortar) {
      const px = x + off, py = y;
      if (mode === 'albedo') {
        // clay brick with per-brick tonal variation — reds, browns, the odd dark
        const base = [[150, 74, 56], [126, 62, 48], [168, 92, 70], [110, 70, 60]][Math.floor(Math.random() * 4)];
        const v = (Math.random() - 0.5) * 22;
        g.fillStyle = `rgb(${base[0] + v},${base[1] + v},${base[2] + v})`;
        g.fillRect(px, py, bw - mortar, bh - mortar);
        // subtle stipple on the face
        g.fillStyle = 'rgba(0,0,0,0.05)';
        for (let k = 0; k < 12; k++) g.fillRect(px + Math.random() * bw, py + Math.random() * bh, 2, 2);
      } else {
        // brick face is raised (light), a bevel of mortar around it
        g.fillStyle = '#c8c8c8';
        g.fillRect(px + 1, py + 1, bw - mortar - 2, bh - mortar - 2);
      }
    }
    row++;
  }
...
function concreteCanvas(mode) {
  const S = 512, c = canvas2d(S), g = c.getContext('2d');
  if (mode === 'albedo') {
    g.drawImage(fbmCanvas(S, [4, 8], 0.42, 0.58), 0, 0);
    g.globalCompositeOperation = 'multiply'; g.fillStyle = '#9a9c9e'; g.fillRect(0, 0, S, S);
    g.globalCompositeOperation = 'source-over';
  } else {
    g.drawImage(fbmCanvas(S, [4, 8], 0.46, 0.54), 0, 0);
  }
  // form-tie holes and panel seams — exposed concrete has a grid of them
  const pxPerIn = S / WALL_TEX_IN, step = 24 * pxPerIn;
  g.fillStyle = mode === 'albedo' ? 'rgba(0,0,0,0.28)' : '#2a2a2a';
  for (let y = step / 2; y < S; y += step) for (let x = step / 2; x < S; x += step) {
    g.beginPath(); g.arc(x, y, 2.5, 0, 7); g.fill();
  }
  g.strokeStyle = mode === 'albedo' ? 'rgba(0,0,0,0.12)' : 'rgba(120,120,120,0.6)';
  g.lineWidth = mode === 'albedo' ? 1 : 3;
  for (let y = 0; y <= S; y += step) { g.beginPath(); g.moveTo(0, y); g.lineTo(S, y); g.stroke(); }
  return c;
}
function woodCanvas(mode) {
  const S = 512, c = canvas2d(S), g = c.getContext('2d');
  const pxPerIn = S / WALL_TEX_IN, plank = 6 * pxPerIn;
  if (mode === 'albedo') { g.fillStyle = '#7a5636'; g.fillRect(0, 0, S, S); }
  else { g.fillStyle = '#808080'; g.fillRect(0, 0, S, S); }
  for (let x = 0; x < S; x += plank) {
    if (mode === 'albedo') {
      const v = (Math.random() - 0.5) * 26;
      g.fillStyle = `rgb(${122 + v},${86 + v},${54 + v})`;
      g.fillRect(x, 0, plank - 1, S);
      // grain lines
      g.strokeStyle = 'rgba(60,40,24,0.35)'; g.lineWidth = 1;
      for (let k = 0; k < 6; k++) { const gx = x + 4 + Math.random() * (plank - 8);
        g.beginPath(); g.moveTo(gx, 0); for (let yy = 0; yy < S; yy += 20) g.lineTo(gx + Math.sin(yy * 0.05 + x) * 2, yy); g.stroke(); }
    }
    // plank groove (recessed)
    g.strokeStyle = mode === 'albedo' ? 'rgba(0,0,0,0.35)' : '#303030';
...
const WALL_MATERIALS = {
  drywall:  { label: 'Painted drywall', color: 0xcfd4db, rough: 0.94, env: 0.3,  nrm: 0.32, tile: 96,
              albedo: () => wallCanvas('albedo'), height: () => wallCanvas('height') },
  brick:    { label: 'Exposed brick',   color: 0xffffff, rough: 0.96, env: 0.18, nrm: 0.9,  tile: 96,
              albedo: () => brickCanvas('albedo'), height: () => brickCanvas('height') },
  concrete: { label: 'Concrete',        color: 0xb8babd, rough: 0.9,  env: 0.25, nrm: 0.5,  tile: 96,
              albedo: () => concreteCanvas('albedo'), height: () => concreteCanvas('height') },
  wood:     { label: 'Wood panel',      color: 0xffffff, rough: 0.7,  env: 0.45, nrm: 0.55, tile: 96,
              albedo: () => woodCanvas('albedo'), height: () => woodCanvas('height') }
};
// Cache one texture set per material so we are not regenerating canvases per wall.
const _wallMatCache = {};
function wallMatAssets(key) {
  const def = WALL_MATERIALS[key] || WALL_MATERIALS.drywall;
  if (_wallMatCache[key]) return _wallMatCache[key];
  const albedo = new THREE.CanvasTexture(def.albedo());
  albedo.colorSpace = THREE.SRGBColorSpace; albedo.wrapS = albedo.wrapT = THREE.RepeatWrapping; albedo.anisotropy = 8;
  const normal = normalFromHeight(def.height(), def.nrm);
  return (_wallMatCache[key] = { def, albedo, normal });
}
...
function makeWallMaterial(len, h, material) {
  const key = WALL_MATERIALS[material] ? material : 'drywall';
  const { def, albedo, normal } = wallMatAssets(key);
  const m = new THREE.MeshStandardMaterial({
    color: def.color, map: albedo.clone(), normalMap: normal.clone(),
    normalScale: new THREE.Vector2(1, 1),
    roughness: def.rough, metalness: 0, envMapIntensity: def.env,
    transparent: true, opacity: 1
  });
  m.map.needsUpdate = true; m.normalMap.needsUpdate = true;
  for (const t of [m.map, m.normalMap]) { t.wrapS = t.wrapT = THREE.RepeatWrapping; t.repeat.set(len / def.tile, h / def.tile); }
...
// The three.js build here ships without BufferGeometryUtils, so this is a
// minimal merge: everything is made non-indexed (box geometry is indexed) and
// its position/normal/uv streams are concatenated. Enough to fuse the wall
// segments around a window or door into one mesh that still raycasts and
// textures as a single wall.
...
// Standard rough openings, in inches.
const OPENING_SIZES = {
  window: { w: 36, h: 48, sill: 36 },
  door:   { w: 36, h: 84, sill: 0 },
  storefront: { w: 72, h: 84, sill: 0 },   // big shop-front glass, floor to head
};
function wallOpenings(w) { return (w.openings || []).filter(o => o && o.kind); }

// The wall body as solid boxes around every opening: full-height piers between
// openings, a header over each, and a sill under each window. Returns a merged
// geometry (or a plain box when there are no openings), with UVs planar-projected
// onto the wall face so brick tiles continuously across piers and headers.
function wallBodyGeometry(w, len, h) {
  const ops = wallOpenings(w).map(o => {
    const std = OPENING_SIZES[o.kind] || OPENING_SIZES.window;
    const width = o.w || std.w, height = o.h || std.h, sill = o.sill !== undefined ? o.sill : std.sill;
    const cx = -len / 2 + (o.at !== undefined ? o.at : 0.5) * len;
    return { x1: Math.max(-len / 2, cx - width / 2), x2: Math.min(len / 2, cx + width / 2),
      y1: -h / 2 + sill, y2: -h / 2 + sill + height };
  }).filter(o => o.x2 > o.x1).sort((a, b) => a.x1 - b.x1);
  if (!ops.length) return new THREE.BoxGeometry(len, h, WALL_T);

  const boxes = [];
  const addBox = (x1, x2, y1, y2) => {
    if (x2 - x1 < 0.1 || y2 - y1 < 0.1) return;
    const g = new THREE.BoxGeometry(x2 - x1, y2 - y1, WALL_T);
    g.translate((x1 + x2) / 2, (y1 + y2) / 2, 0);
    boxes.push(g);
  };
  let cursor = -len / 2;
  for (const o of ops) {
    if (o.x1 > cursor) addBox(cursor, o.x1, -h / 2, h / 2);   // full-height pier before the opening
    addBox(o.x1, o.x2, o.y2, h / 2);                          // header above
    if (o.y1 > -h / 2) addBox(o.x1, o.x2, -h / 2, o.y1);      // sill below (windows)
    cursor = Math.max(cursor, o.x2);
  }
  if (cursor < len / 2) addBox(cursor, len / 2, -h / 2, h / 2);   // pier after the last opening
  const merged = mergeGeoms(boxes);
  // planar UVs across the whole wall face so the texture is continuous
  const p = merged.attributes.position.array, uv = new Float32Array(merged.attributes.position.count * 2);
  for (let i = 0, j = 0; i < p.length; i += 3, j += 2) {
    uv[j] = (p[i] + len / 2) / len; uv[j + 1] = (p[i + 1] + h / 2) / h;
  }
  merged.setAttribute('uv', new THREE.BufferAttribute(uv, 2));
  return merged;
}

// Frame + glass (window) or frame + leaf (door), built in the wall's local
// space and added as children of the wall body. Cosmetic — not raycast for
// wall hits, which keeps placement/drilling working exactly as before.
const glassMat = new THREE.MeshPhysicalMaterial({
  color: 0xaecbe0, roughness: 0.05, metalness: 0, transmission: 0.85, transparent: true,
  opacity: 0.34, ior: 1.5, envMapIntensity: 1, thickness: 1, side: THREE.DoubleSide
});
const frameMatWhite = new THREE.MeshStandardMaterial({ color: 0xf2f3f5, roughness: 0.5, metalness: 0.1 });
const doorLeafMat = new THREE.MeshStandardMaterial({ color: 0xb9942f, roughness: 0.55, metalness: 0.05 });
function buildOpeningTrim(w, len, h) {
  const grp = new THREE.Group();
  for (const o of wallOpenings(w)) {
    const std = OPENING_SIZES[o.kind] || OPENING_SIZES.window;
    const width = o.w || std.w, height = o.h || std.h, sill = o.sill !== undefined ? o.sill : std.sill;
    const cx = -len / 2 + (o.at !== undefined ? o.at : 0.5) * len;
    const cy = -h / 2 + sill + height / 2;
    const fr = 1.5, fd = WALL_T + 1;                     // frame thickness / depth
    // frame: four bars around the opening
    const bar = (bw, bh, x, y) => { const m = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, fd), frameMatWhite);
      m.position.set(x, y, 0); m.castShadow = true; grp.add(m); };
    bar(width + fr * 2, fr, cx, cy + height / 2);        // head
    bar(width + fr * 2, fr, cx, cy - height / 2);        // sill/threshold
    bar(fr, height, cx - width / 2, cy);                 // left jamb
    bar(fr, height, cx + width / 2, cy);                 // right jamb
    if (o.kind === 'door') {
      // a leaf hinged on the left, opened by o.open (0 closed .. 1 ~90°)
      const leaf = new THREE.Group();
      const panel = new THREE.Mesh(new THREE.BoxGeometry(width - 1, height - 1, 1.4), doorLeafMat);
      panel.position.set((width - 1) / 2, 0, 0); panel.castShadow = true;
      const knob = new THREE.Mesh(new THREE.SphereGeometry(0.9, 12, 12), mat(0xcaa64a, { metalness: 0.9, roughness: 0.3 }));
      knob.position.set(width - 4, 0, 1.1); leaf.add(panel); leaf.add(knob);
      leaf.position.set(cx - width / 2, cy, 0);
      leaf.rotation.y = -(o.open || 0) * Math.PI * 0.5;
      leaf.userData = { isDoorLeaf: true, wallId: w.id, openingId: o.id };
      grp.add(leaf);
    } else {
      // glass pane, plus a cross mullion so it reads as a real window
      const glass = new THREE.Mesh(new THREE.PlaneGeometry(width - 1, height - 1), glassMat);
      glass.position.set(cx, cy, 0); grp.add(glass);
      const mv = new THREE.Mesh(new THREE.BoxGeometry(0.7, height - 1, WALL_T + 0.4), frameMatWhite);
      mv.position.set(cx, cy, 0); grp.add(mv);
      const mh = new THREE.Mesh(new THREE.BoxGeometry(width - 1, 0.7, WALL_T + 0.4), frameMatWhite);
      mh.position.set(cx, cy, 0); grp.add(mh);
    }
  }
  return grp;
}
...
function buildWall(w) {
  ...
  const h = w.h || WALL_H;
  const m = new THREE.Mesh(wallBodyGeometry(w, len, h), makeWallMaterial(len, h, w.material));
  m.position.set((w.x1 + w.x2) / 2, (w.y0 || 0) + h / 2, (w.z1 + w.z2) / 2);
  m.rotation.y = Math.atan2(-dz, dx);
  m.castShadow = true;
  m.receiveShadow = true;
  m.userData = { isWall: true, wallId: w.id };
  if (wallOpenings(w).length) m.add(buildOpeningTrim(w, len, h));
...
// Dispose a wall's mesh (and cosmetic trim children) without touching its holes
// or removing it from state — used by rebuildWall.
function disposeWallMesh(id) {
  ...
  m.material.dispose();
  m.traverse(o => { if (o.geometry && o !== m) o.geometry.dispose(); });
...
// polygonOffset pulls the slab a hair toward the camera in DEPTH ONLY, without
// moving it in space. A slab drawn at grade sits in exactly the same plane as
// the infinite ground, and two coplanar surfaces fight for the depth buffer —
// that is the flickering/striping through the floor. Offsetting the slab makes
// it win cleanly at every angle and distance, which nudging the geometry up by
// a fraction of an inch does not (the fight comes back as the camera pulls out).
const slabMat = new THREE.MeshStandardMaterial({ color: 0xb9bec7, map: makeSlabTexture(),
...
// ---- Level of detail ----
// A 48-port switch is ~150 meshes, nearly all of it port jewellery: bezel,
// jack, gold contacts, link LED per port. Twenty filled racks measured 146,842
// meshes and 33,952 draw calls at 81 ms/frame; hundreds of racks is impossible
// that way. But the faceplate TEXTURE already draws the ports, so from more
// than a few feet away the modelled jacks add nothing you can see.
//
// So the detail is reparented into one child group per device and that group's
// visibility is driven by camera distance. Crucially this HIDES rather than
// deletes: three.js skips an invisible subtree entirely when rendering, while
// getWorldPosition still returns correct transforms — so cable endpoints,
// which resolve through port meshes, keep working at any distance.
...
function lodPackDevice(g) {
  ...
  detail.name = '__detail';
  for (const child of [...g.children]) {
    const u = child.userData || {};
    if (u.isPort || u.portLed || child.userData.__detail) { detail.add(child); continue; }
    // bezels/contacts carry no flags — identify them by shared detail geometry
    if (child.isMesh && (child.geometry === GEO_RJ_BEZEL || child.geometry === GEO_SFP_BEZEL ||
        child.geometry === GEO_GOLD || child.geometry === GEO_LED)) detail.add(child);
  }
  if (detail.children.length) { g.add(detail); g.userData.detail = detail; }
}
let _lodLast = 0;
function updateDeviceLOD(force) {
  ...
  _lodLast = now;
  const near2 = LOD_DETAIL_IN * LOD_DETAIL_IN;
  for (const g of deviceGroups.values()) {
    const d = g.userData.detail;
    if (!d) continue;
    g.getWorldPosition(_lodTmp);
    const want = _lodTmp.distanceToSquared(camera.position) < near2;
    if (d.visible !== want) d.visible = want;
  }
}
...
// A subnet mask is contiguous ones followed by contiguous zeros. 255.0.255.0
// parses as an address but is not a mask, and real gear refuses it.
function validMask(str) {
  const m = parseIp(str);
  if (!m) return null;
  const mi = m.int >>> 0, host = (~mi) >>> 0;
  if (((host + 1) & host) !== 0) return null;
  return maskToCidr(str);
}
// IEEE 802.3 rates an Ethernet port can be forced to. Anything else is not a
// speed, it is a typo.
const ETH_SPEEDS = [10, 100, 1000, 2500, 5000, 10000, 25000, 40000, 100000];
function vlanIdOk(v) { return Number.isInteger(v) && v >= 1 && v <= 4094; }
...
function deviceInterfaces(dev) {
  ...
  // A managed switch's address lives in dev.ip — that is its MANAGEMENT
  // interface, the real box's Vlan1/me0. Leaving it out made a switch look like
  // it had no IP stack at all: it could not be pinged, could not ping, and did
  // not appear in `show ip int brief`, even though the map clearly gave it an
  // address. Routers/gateways carry their addresses on ports and SVIs, so this
  // only fills the gap for a device whose sole address is dev.ip.
  const cls = netClass(dev);
...
//////////////////// OSPF (RFC 2328) ////////////////////
// A link-state protocol: every router floods what it is connected to, all of
// them build the same map, and each runs Dijkstra from itself. That is a real
// difference from static routes — routes appear without being typed, and they
// disappear when a link goes down, which is the whole point of running it.
//
// Cost = reference bandwidth / interface bandwidth, minimum 1. Cisco's default
// reference is 100 Mbps (10^8 bits/s), which is why every link at 100 M and
// above costs 1 unless you raise it with `auto-cost reference-bandwidth` —
// verified against Cisco's own docs and two further references. That default
// is a genuine trap in real networks and the panel says so.
const OSPF_REF_BW_MBPS = 100;
function ospfEnabled(dev) { return !!(dev.ospf && dev.ospf.enabled); }
function ospfRefBw(dev) { return (dev.ospf && +dev.ospf.refBw) || OSPF_REF_BW_MBPS; }
function ospfCost(dev, speedGbps) {
  const mbps = (speedGbps || 1) * 1000;
  return Math.max(1, Math.round(ospfRefBw(dev) / mbps));
}
// Router ID: configured, else the highest loopback, else the highest interface
// address — RFC 2328 §8.1 and Cisco's selection order.
function ospfRouterId(dev) {
  if (dev.ospf && dev.ospf.routerId) { const p = parseIp(dev.ospf.routerId); if (p) return p.int; }
  let best = 0;
  for (const i of deviceInterfaces(dev)) if (i.ip.int > best) best = i.ip.int;
  return best;
}
// Which area an interface is in. `network <addr> <wild> area N` in IOS; here the
// area is per interface, which is the same thing expressed directly.
function ospfAreaOf(dev, iface) {
  const per = dev.ospf && dev.ospf.areas;
  if (per) {
    for (const k of ifKeyAliases(iface)) if (per[k] !== undefined) return +per[k];
  }
  return (dev.ospf && +dev.ospf.area) || 0;
}
// An interface can be named three ways and all three are natural to type:
// the bare port number (what ifKey returns), "Port 3" (what deviceInterfaces
// calls it), and "Vlan20" for an SVI. Config that only matched one of them
// silently did nothing — passive-interface and per-interface areas both looked
// applied and were not.
// What to print. ifKey is the lookup key; this is the human name.
function ifName(iface) {
  if (!iface) return '—';
  return iface.kind === 'svi' ? `Vlan${iface.vlan}` : `Port ${iface.port}`;
}
function ifKeyAliases(iface) {
  if (!iface) return [];
  if (iface.kind === 'svi') return [`Vlan${iface.vlan}`, iface.vlan, String(iface.vlan)];
  return [iface.port, String(iface.port), `Port ${iface.port}`, `port ${iface.port}`];
}
// An interface participates unless it is passive — a passive interface still
// advertises its subnet but never forms an adjacency, which is exactly what you
// want on a user VLAN and is the most common OSPF hardening step.
function ospfPassive(dev, iface) {
  const p = dev.ospf && dev.ospf.passive;
  if (!p) return false;
  const keys = ifKeyAliases(iface);
  if (Array.isArray(p)) return p.some(x => keys.includes(x) || keys.includes(String(x)));
  return keys.some(k => !!p[k]);
}

// Adjacencies: two OSPF routers that share a subnet and can see each other at
// layer 2 become neighbours, unless either side is passive or the areas differ.
// Mismatched areas on a link is a real and commonly-hit misconfiguration, so it
// is reported rather than silently ignored.
function ospfNeighbors() {
  const routers = state.devices.filter(d => isPlaced(d) && ospfEnabled(d));
  const out = [], problems = [];
  for (let i = 0; i < routers.length; i++) {
    for (let j = i + 1; j < routers.length; j++) {
      const A = routers[i], B = routers[j];
      for (const ia of deviceInterfaces(A)) {
        for (const ib of deviceInterfaces(B)) {
          if (!sameSubnet(ia.ip, ib.ip)) continue;
          if (!ospfSegmentConnected(A, ia, B, ib)) continue;
          if (ospfPassive(A, ia) || ospfPassive(B, ib)) continue;
          const aa = ospfAreaOf(A, ia), ab = ospfAreaOf(B, ib);
          if (aa !== ab) {
            problems.push(`${A.name} ${ifName(ia)} is in area ${aa} but ${B.name} ${ifName(ib)} is in area ${ab} — no adjacency forms across an area mismatch`);
            continue;
          }
          const speed = ospfLinkSpeed(A, ia, B, ib);
          out.push({ a: A, ia, b: B, ib, area: aa,
            costA: ospfCost(A, speed), costB: ospfCost(B, speed), speed });
        }
      }
    }
  }
  return { adj: out, problems };
}
// Do these two interfaces actually share a wire? An address match is not enough
// — two routers can be numbered in the same subnet and not be cabled together.
function ospfSegmentConnected(A, ia, B, ib) {
  const def = DEVICE_TYPES[A.type];
  for (let p = 1; p <= (def.ports || 0); p++) {
    if (portRole(def, p) === 'PWR') continue;
    if (ia.kind === 'routed' && ia.port !== p) continue;
    const vlan = ia.kind === 'svi' ? ia.vlan : undefined;
    if (vlan !== undefined && !portCarries(A, p, vlan)) continue;
    const walk = l2Walk(A, { startPort: p, vlan, ignoreVlan: vlan === undefined });
    if (walk.routers.has(B.id)) return true;
  }
  return false;
}
function ospfLinkSpeed(A, ia, B, ib) {
  // The slower end of the physical link, negotiated.
  let best = null;
  for (const c of state.cables) {
    const da = deviceById(c.a.deviceId), db = deviceById(c.b.deviceId);
    if (!da || !db) continue;
    const pair = (da.id === A.id && db.id === B.id) || (da.id === B.id && db.id === A.id);
    if (!pair) continue;
    const sp = linkSpeed(c);
    if (best === null || sp < best) best = sp;
  }
  return best === null ? 1 : best;
}

// Run SPF from one router and return the routes it learns.
function ospfRoutes(dev) {
  if (!ospfEnabled(dev)) return [];
  const { adj } = ospfNeighbors();
  if (!adj.length) return [];
  // Build the graph. Nodes are routers; an edge carries the cost of leaving a
  // router by that link, which is why cost is directional.
  const graph = new Map();
  const addEdge = (from, to, cost, viaIp, viaIface) => {
    if (!graph.has(from)) graph.set(from, []);
    graph.get(from).push({ to, cost, viaIp, viaIface });
  };
  for (const a of adj) {
    addEdge(a.a.id, a.b.id, a.costA, a.ib.ip.int, a.ia);
    addEdge(a.b.id, a.a.id, a.costB, a.ia.ip.int, a.ib);
  }
  // Dijkstra from this router.
  const dist = new Map([[dev.id, 0]]);
  const prev = new Map();
  const todo = new Set([dev.id, ...graph.keys()]);
  for (const e of adj) { todo.add(e.a.id); todo.add(e.b.id); }
  while (todo.size) {
    let cur = null;
    for (const id of todo) if (cur === null || (dist.get(id) ?? Infinity) < (dist.get(cur) ?? Infinity)) cur = id;
    if (cur === null || (dist.get(cur) ?? Infinity) === Infinity) break;
    todo.delete(cur);
    for (const e of graph.get(cur) || []) {
      if (!todo.has(e.to)) continue;
      const nd = dist.get(cur) + e.cost;
      if (nd < (dist.get(e.to) ?? Infinity)) {
        dist.set(e.to, nd);
        // The next hop is the first hop out of THIS router, carried forward —
        // a route two hops away still points at the neighbour, not the far end.
        const first = cur === dev.id ? e : prev.get(cur).first;
        prev.set(e.to, { from: cur, edge: e, first });
      }
    }
  }
  // Every subnet on every reachable router becomes a route.
  const routes = [];
  const mine = new Set(deviceInterfaces(dev).map(i => `${i.ip.network}/${i.ip.cidr}`));
  for (const [rid, cost] of dist) {
    if (rid === dev.id) continue;
    const r = deviceById(rid);
    if (!r) continue;
    const hop = prev.get(rid);
    if (!hop) continue;
    for (const i of deviceInterfaces(r)) {
      const key = `${i.ip.network}/${i.ip.cidr}`;
      if (mine.has(key)) continue;                    // connected wins, always
      if (ospfPassive(r, i) && false) continue;
      routes.push({ prefix: i.ip.network >>> 0, cidr: i.ip.cidr, mask: i.ip.mask >>> 0,
        via: hop.first.viaIp, iface: ifKey(hop.first.viaIface), ifRef: hop.first.viaIface,
        ad: AD.ospf, metric: cost, src: 'ospf', code: AD_CODE.ospf,
        area: ospfAreaOf(r, i), learnedFrom: r.name });
    }
  }
  // Keep the lowest metric per prefix — equal-cost paths would both install on
  // a real box, but the table shows one line per prefix here.
  const best = new Map();
  for (const r of routes) {
    const k = `${r.prefix}/${r.cidr}`;
    if (!best.has(k) || r.metric < best.get(k).metric) best.set(k, r);
  }
  return [...best.values()];
}

// `show ip ospf neighbor`
function showIpOspfNeighbor(dev) {
  if (!ospfEnabled(dev)) return `${dev.name}: OSPF is not enabled`;
  const { adj, problems } = ospfNeighbors();
  const mine = adj.filter(a => a.a.id === dev.id || a.b.id === dev.id);
  const lines = [`Router ID ${ipStr(ospfRouterId(dev))} · reference bandwidth ${ospfRefBw(dev)} Mbps`, ''];
  if (!mine.length) lines.push('No neighbours — no other OSPF router shares a segment with this one.');
  else {
    lines.push('Neighbor ID       Pri  State     Address          Interface   Cost');
    for (const a of mine) {
      const far = a.a.id === dev.id ? a.b : a.a;
      const farIf = a.a.id === dev.id ? a.ib : a.ia;
      const myIf = a.a.id === dev.id ? a.ia : a.ib;
      const cost = a.a.id === dev.id ? a.costA : a.costB;
      lines.push(`${ipStr(ospfRouterId(far)).padEnd(17)} 1    FULL      ${ipStr(farIf.ip.int).padEnd(16)} ${ifName(myIf).padEnd(11)} ${cost}`);
    }
  }
...
function routingTable(dev) {
  ...
  // Learned routes. They lose to connected (AD 0) and static (AD 1) on the same
  // prefix, which is what administrative distance is for.
  for (const r of ospfRoutes(dev)) out.push(r);
...
function updateHover(cx, cy) {
  ...
  if (mode === 'place' && pendingType) {
    ...
    if (def.field) {
      ...
      // Set-on-surface: point at the TOP of any furniture or device (a desk, a
      // shelf, a cabinet, even another switch) and the device sits on it. This
      // was the "big nono" — you could not put a modem on a desk or an AP on a
      // cabinet. A top face is one whose world normal points mostly up.
      const surf = firstHit(collectDeviceBodies());
      if (surf) {
        const n = surf.face.normal.clone().transformDirection(surf.object.matrixWorld).normalize();
        const underId = surf.object.userData.deviceId;
        const under = deviceById(underId);
        if (n.y > 0.7 && underId !== placeExistingId && under) {
          const topY = surf.point.y;
          makeGhost(8, 8, 8, true);
          ghost.position.set(surf.point.x, topY + 4, surf.point.z);
          ghost.rotation.y = 0;
          hoverInfo = { mountKind: 'desk', x: surf.point.x, z: surf.point.z, y0: topY, onId: underId };
          setStatus(`${def.label} — set on ${DEVICE_TYPES[under.type].label}. Click to place.`);
          return;
        }
      }
...
//////////////////// Doors ////////////////////
// A door on a security plan is not decoration. It is either open or it is not,
// it is either locked or it is not, and a mantrap is two doors that are
// electrically forbidden from being open at the same time. Modelling that is
// what makes a badge reader mean something.

const DOOR_SWING_RATE = 2.2;              // radians-ish per second, ~0.7 s to swing
const _doorAnim = new Map();              // "wallId:openingId" -> current 0..1

function doorKey(wallId, openingId) { return `${wallId}:${openingId}`; }

function eachDoor(fn) {
  for (const w of state.walls || []) {
    for (const o of w.openings || []) {
      if (o.kind === 'door' || o.kind === 'storefront') fn(w, o);
    }
  }
}

function findDoor(wallId, openingId) {
  const w = (state.walls || []).find(x => x.id === wallId);
  const o = w && (w.openings || []).find(x => x.id === openingId);
  return o ? { wall: w, opening: o } : null;
}

// Two doors in the same interlock group cannot both be open. That is a mantrap:
// the outer door has to shut before the inner one will release.
function interlockBlocks(wall, opening) {
  const grp = opening.interlock;
  if (!grp) return null;
  let blocker = null;
  eachDoor((w2, o2) => {
    if (blocker || o2.id === opening.id || o2.interlock !== grp) return;
    const cur = _doorAnim.get(doorKey(w2.id, o2.id));
    if ((cur !== undefined ? cur : (o2.open || 0)) > 0.02 || o2.wantOpen) blocker = o2;
  });
  return blocker;
}

// Returns why it refused, or null when it opened.
function setDoorOpen(wallId, openingId, want) {
  const hit = findDoor(wallId, openingId);
  if (!hit) return 'no such door';
  const { wall, opening } = hit;
  if (want) {
    if (opening.locked) return `${opening.label || 'This door'} is locked — badge in to release it`;
    const b = interlockBlocks(wall, opening);
    if (b) return `Interlocked: ${b.label || 'the other door'} must close first`;
  }
  opening.wantOpen = !!want;
  return null;
}

function toggleDoor(wallId, openingId) {
  const hit = findDoor(wallId, openingId);
  if (!hit) return;
  const cur = _doorAnim.get(doorKey(wallId, openingId));
  const isOpen = (cur !== undefined ? cur : (hit.opening.open || 0)) > 0.5;
  const why = setDoorOpen(wallId, openingId, !isOpen);
  setStatus(why || `${hit.opening.label || 'Door'} ${!isOpen ? 'opening' : 'closing'}.`);
}

// Ease every door toward what it has been told to do, and rotate the leaf that
// is already in the scene rather than rebuilding the wall — a door that
// rebuilds geometry every frame would be unusable.
function animateDoors(dt) {
  if (HEADLESS) return;
  let any = false;
  eachDoor((w, o) => {
    const key = doorKey(w.id, o.id);
    const target = o.wantOpen ? 1 : 0;
    let cur = _doorAnim.has(key) ? _doorAnim.get(key) : (o.open || 0);
    if (Math.abs(cur - target) < 0.001) { _doorAnim.set(key, target); return; }
    const step = DOOR_SWING_RATE * dt;
    cur = cur < target ? Math.min(target, cur + step) : Math.max(target, cur - step);
    _doorAnim.set(key, cur);
    o.open = cur;
    any = true;
    const g = wallMeshes.get(w.id);
    if (!g) return;
    g.traverse(obj => {
      if (obj.userData && obj.userData.isDoorLeaf && obj.userData.openingId === o.id) {
        obj.rotation.y = -cur * Math.PI * 0.5;
      }
    });
  });
  if (any) markShadowsDirty();
}

// In walk mode a door you push against opens by itself, the way one does when
// you have a badge. Locked ones do not, and say so.
let _doorNagAt = 0;
function autoDoorsForWalker() {
  if (!walkActive) return;
  const p = camera.position;
  eachDoor((w, o) => {
    const std = OPENING_SIZES[o.kind] || OPENING_SIZES.door;
    const len = Math.hypot(w.x2 - w.x1, w.z2 - w.z1) || 1;
    const t = (o.at !== undefined ? o.at : 0.5);
    const dx = (w.x2 - w.x1) / len, dz = (w.z2 - w.z1) / len;
    const cx = w.x1 + dx * len * t, cz = w.z1 + dz * len * t;
    const near = Math.hypot(p.x - cx, p.z - cz) < (o.w || std.w) * 0.9 + 26;
    if (near && !o.wantOpen) {
      const why = setDoorOpen(w.id, o.id, true);
      if (why && performance.now() - _doorNagAt > 2500) { setStatus(why); _doorNagAt = performance.now(); }
    } else if (!near && o.wantOpen && o.autoClose !== false) {
      setDoorOpen(w.id, o.id, false);
    }
  });
}

function handleClick() {
  // A door is a thing you operate. Clicking one should work it, not select the
  // wall behind it — so it is picked before anything else.
  if (mode === 'select') {
    const hits = raycaster.intersectObjects([...wallMeshes.values()], true);
    for (const h of hits) {
      let o = h.object;
      while (o && !(o.userData && o.userData.isDoorLeaf)) o = o.parent;
      if (o) { toggleDoor(o.userData.wallId, o.userData.openingId); return; }
    }
  }
  if (mode === 'place' && pendingType) {
    if (!hoverInfo) return;
  ...
  if (mode === 'wall') {
    ...
    if (!wallStart) {
      ...
      undoPush();
      const w = { id: uid(), x1: wallStart.x, z1: wallStart.z, x2: hoverInfo.x, z2: hoverInfo.z, h: WALL_H, y0: wallStart.y0 || 0, material: pendingWallMaterial };
      state.walls.push(w);
      buildWall(w);
...
// OSPF config. Enabling it and picking an area is most of what people type; the
...
function ospfPropsHtml(dev) {
  ...
  return `
    <div class="row" style="margin-top:10px"><span class="k">OSPF</span>
      <input type="checkbox" id="dev-ospf-on" ${on ? 'checked' : ''}></div>
    <div id="dev-ospf-cfg" style="${on ? '' : 'display:none'}">
      <div class="row"><span class="k">router-id</span>
        <input type="text" id="os-rid" style="width:118px" value="${esc(o.routerId || '')}"
          placeholder="${ipStr(ospfRouterId(dev))} (auto)"></div>
      <div class="row"><span class="k">area (default)</span>
        <input type="number" id="os-area" style="width:60px" value="${o.area === undefined ? 0 : o.area}"></div>
      <div class="row"><span class="k">reference bw</span>
        <input type="number" id="os-ref" style="width:74px" value="${o.refBw || OSPF_REF_BW_MBPS}">
        <span class="unit">Mbps</span></div>
      ${o.refBw ? '' : '<p class="cbl-edit-hint">At the 100 Mbps default every link 100 M and faster costs 1 — raise this to 10000 to tell 1 G and 10 G apart.</p>'}
      <div class="row"><span class="k">per interface</span></div>
      ${ifaces.map(i => {
        const k = ifKey(i);
        const area = ospfAreaOf(dev, i);
        return `<div class="row" data-ospfif="${esc(String(k))}">
          <span class="k" style="font-size:11px">${ifName(i)}</span>
          <input type="number" class="os-ifarea" style="width:54px" value="${area}" title="area">
          <label style="font-size:11px"><input type="checkbox" class="os-ifpass" ${ospfPassive(dev, i) ? 'checked' : ''}> passive</label>
          <span class="ai-meta">cost ${ospfCost(dev, 1)}</span></div>`;
      }).join('')}
      <div class="row" style="margin-top:8px"><span class="k">show ip ospf neighbor</span></div>
      <pre style="font-size:10px;white-space:pre-wrap;margin:2px 0">${esc(showIpOspfNeighbor(dev))}</pre>
    </div>`;
}
function wireOspfProps(dev) {
  const cb = document.getElementById('dev-ospf-on');
  if (!cb) return;
  const root = propsBody;
  const save = () => {
    const g = id => (document.getElementById(id) || {}).value;
    const areas = {}, passive = [];
    for (const el of root.querySelectorAll('[data-ospfif]')) {
      const k = el.dataset.ospfif;
      const a = el.querySelector('.os-ifarea').value;
      if (a !== '') areas[k] = +a;
      if (el.querySelector('.os-ifpass').checked) passive.push(isNaN(+k) ? k : +k);
    }
    dev.ospf = { enabled: cb.checked, routerId: (g('os-rid') || '').trim(),
      area: +g('os-area') || 0, refBw: +g('os-ref') || OSPF_REF_BW_MBPS, areas, passive };
    flushL2Tables();
    showDeviceProps(dev.id);
  };
  cb.onchange = save;
  for (const el of root.querySelectorAll('#dev-ospf-cfg input')) el.onchange = save;
}
...
function showWallProps(id) {
  ...
  const curMat = WALL_MATERIALS[w.material] ? w.material : 'drywall';
  const ops = w.openings || [];
  const opRows = ops.map((o, i) =>
    `<div class="row" data-op="${i}" style="gap:5px">
       <span class="k" style="font-size:11px;text-transform:capitalize">${o.kind}</span>
       <input type="range" class="op-at" min="5" max="95" value="${Math.round((o.at !== undefined ? o.at : 0.5) * 100)}" style="flex:1">
       ${o.kind === 'door' ? `<button class="op-swing" title="Open/close">${o.open ? 'Close' : 'Open'}</button>` : ''}
       <button class="op-del" title="Remove">×</button></div>`).join('');
  propsBody.innerHTML = `
    ${row('Length', (len / 12).toFixed(1) + ' ft')}
    ${row('Height', ((w.h || WALL_H) / 12).toFixed(1) + ' ft')}
    <div class="row"><span class="k">Material</span>
      <select id="wall-mat" style="width:130px">${Object.entries(WALL_MATERIALS).map(([k, d]) =>
        `<option value="${k}" ${k === curMat ? 'selected' : ''}>${d.label}</option>`).join('')}</select></div>
    <div class="row" style="margin-top:8px"><span class="k">Openings</span></div>${opRows}
    <div class="row" style="gap:6px;flex-wrap:wrap">
      <button id="wall-add-win">+ Window</button>
      <button id="wall-add-door">+ Door</button>
      <button id="wall-add-front">+ Storefront</button></div>
    ${row('Drilled holes', holes)}
    <button id="wall-del" class="danger">Delete wall</button>`;
  propsEl.classList.remove('hidden');
  selected = { kind: 'wall', id };
  const matSel = document.getElementById('wall-mat');
  matSel.onchange = () => { w.material = matSel.value; rebuildWall(id); };
  // Add an opening at the least-crowded position, then rebuild the wall.
  const addOpening = (kind) => {
    w.openings = w.openings || [];
    const used = w.openings.map(o => o.at !== undefined ? o.at : 0.5);
    let at = 0.5; let best = -1;
    for (const cand of [0.5, 0.25, 0.75, 0.15, 0.85, 0.4, 0.6]) {
      const d = Math.min(1, ...used.map(u => Math.abs(u - cand)));
      if (d > best) { best = d; at = cand; }
    }
    w.openings.push({ id: uid(), kind, at });
    undoPush(); rebuildWall(id); showWallProps(id);
  };
  document.getElementById('wall-add-win').onclick = () => addOpening('window');
  document.getElementById('wall-add-door').onclick = () => addOpening('door');
  document.getElementById('wall-add-front').onclick = () => addOpening('storefront');
  for (const el of propsBody.querySelectorAll('[data-op]')) {
    const i = +el.dataset.op;
    const at = el.querySelector('.op-at');
    if (at) at.oninput = () => { w.openings[i].at = +at.value / 100; rebuildWall(id); };
    const del = el.querySelector('.op-del');
    if (del) del.onclick = () => { w.openings.splice(i, 1); undoPush(); rebuildWall(id); showWallProps(id); };
    const sw = el.querySelector('.op-swing');
    if (sw) sw.onclick = () => { w.openings[i].open = w.openings[i].open ? 0 : 1; rebuildWall(id); showWallProps(id); };
  }
...
(function initWallMatUI() {
  const sel = document.getElementById('wall-mat-sel');
  if (!sel) return;
  sel.innerHTML = Object.entries(WALL_MATERIALS).map(([k, d]) =>
    `<option value="${k}" ${k === 'drywall' ? 'selected' : ''}>${d.label}</option>`).join('');
  sel.onchange = () => { pendingWallMaterial = sel.value; };
})();
...
// UniFiOS hosts several apps under one shell. Network is the working one;
// Protect and Access are real device-management surfaces driven by the cameras
// and door hardware actually placed in the model — no fabricated events. Talk
// and Connect appear in the switcher as the real console shows them but are not
// modelled yet, and say so plainly rather than faking a screen.
const UF_APPS = [
  { id: 'Network', ico: '⧉' },
  { id: 'Protect', ico: '◉' },
  { id: 'Access',  ico: '⊟' },
...
function openUnifi(dev) {
  ...
  document.getElementById('unifi').classList.remove('hidden');
  ufRender();
}
function ufClose() { const b = document.getElementById('unifi'); if (b) b.classList.add('hidden'); _uf = null; }
function ufSetApp(app) {
  _uf.app = app;
  _uf.page = app === 'Network' ? 'dashboard' : (app === 'Protect' ? 'cameras' : (app === 'Access' ? 'doors' : null));
  ufRender();
}
// Devices flagged by catalog category
function unifiCameras() { return state.devices.filter(d => isPlaced(d) && /UniFi Cameras/.test((DEVICE_TYPES[d.type] || {}).cat || '')); }
function unifiDoorGear() { return state.devices.filter(d => isPlaced(d) && /Door Access/.test((DEVICE_TYPES[d.type] || {}).cat || '')); }
...
const UF_NAV_PROTECT = [
  { sep: 'PROTECT' },
  { id: 'cameras', ico: '◉', label: 'Cameras' },
  { id: 'recordings', ico: '▤', label: 'Recordings' },
];
const UF_NAV_ACCESS = [
  { sep: 'ACCESS' },
  { id: 'doors', ico: '⊟', label: 'Doors' },
  { id: 'readers', ico: '⌸', label: 'Readers' },
];
function ufRender() {
  if (!_uf) return;
  // app switcher across the top, like the real UniFiOS shell
  const apps = document.getElementById('uf-apps');
  if (apps) {
    apps.innerHTML = UF_APPS.map(a =>
      `<div class="uf-app ${_uf.app === a.id ? 'active' : ''}" data-ufapp="${a.id}"><span class="ico">${a.ico}</span>${a.id}</div>`).join('');
    for (const el of apps.querySelectorAll('[data-ufapp]')) el.onclick = () => ufSetApp(el.dataset.ufapp);
  }
  const appName = document.getElementById('uf-appname');
  if (appName) appName.textContent = _uf.app;
  const navDef = _uf.app === 'Protect' ? UF_NAV_PROTECT : _uf.app === 'Access' ? UF_NAV_ACCESS : UF_NAV;
  const nav = document.getElementById('uf-nav');
  nav.innerHTML = navDef.map(n => n.sep
  ...
  const R = { dashboard: ufDashboard, devices: ufDeviceList, clients: ufClients,
    ports: ufPorts, networks: ufNetworks, wifi: ufWifi, internet: ufInternet,
    cameras: ufCameras, recordings: ufRecordings, doors: ufDoors, readers: ufReaders }[_uf.page];
  top.textContent = (navDef.find(n => n.id === _uf.page) || {}).label || _uf.app;
  page.innerHTML = R ? R() : ufNotModelled();
  ufWire(page);
}
function ufNotModelled() {
  return `<div class="uf-net"><h4>${esc(_uf.app)}</h4>
    <p style="color:#8a9099;font-size:13px">UniFi ${esc(_uf.app)} is part of UniFiOS but is not modelled in the simulation yet.
    Place UniFi ${_uf.app === 'Talk' ? 'phones' : 'devices'} and it will appear here once the device model is built.</p></div>`;
}
// ---- Protect ----
function ufCameras() {
  const cams = unifiCameras();
  if (!cams.length) return `<p style="color:#8a9099">No UniFi cameras placed. Add one from the UniFi Cameras category.</p>`;
  const res = { u_g6bullet: '4K', u_g6turret: '4K', u_g6dome: '4K', u_g6ptz: '4K', u_aipro: '4K', u_ai360: '5MP', u_g5flex: '2K', u_g6instant: '2K' };
  return `<div class="uf-cards">
    <div class="uf-card"><div class="lbl">Cameras</div><div class="big">${cams.length}</div><div class="sub">adopted</div></div>
    <div class="uf-card"><div class="lbl">Recording</div><div class="big">${cams.filter(c=>hostIp(c)).length}</div><div class="sub">online</div></div>
    </div>
    <div class="uf-camgrid">${cams.map(c => {
      const conn = hopThroughPatches(c, hostPort(c), FRONT);
      return `<div class="uf-camcard"><div class="thumb">▣</div>
        <div class="cap">${esc(c.name)}<div class="sub">${res[c.type] || 'HD'} · ${conn ? esc(conn.dev.name) : 'not connected'}</div></div></div>`;
    }).join('')}</div>`;
}
function ufRecordings() {
  const cams = unifiCameras();
  return `<p style="color:#8a9099;font-size:13px;margin-bottom:10px">Retention and storage are device properties that are not modelled yet — this lists what each camera is attached to and whether it can reach the network.</p>
  <table class="sim-table"><tr><th>Camera</th><th>IP</th><th>Switch port</th><th>Status</th></tr>
  ${cams.map(c => { const conn = hopThroughPatches(c, hostPort(c), FRONT);
    return `<tr><td>${esc(c.name)}</td><td>${hostAddr(c)}</td>
      <td>${conn ? esc(conn.dev.name) + ' · ' + conn.port : '—'}</td>
      <td>${conn ? '<span class="uf-dot up"></span>Online' : '<span class="uf-dot down"></span>Offline'}</td></tr>`; }).join('')}</table>`;
}
// ---- Access ----
function ufDoors() {
  const gear = unifiDoorGear();
  if (!gear.length) return `<p style="color:#8a9099">No UniFi Access hardware placed. Add a doorbell, reader or Access Hub from the UniFi Door Access category.</p>`;
  return `<div class="uf-cards">
    <div class="uf-card"><div class="lbl">Access devices</div><div class="big">${gear.length}</div><div class="sub">placed</div></div>
    </div>
    <table class="sim-table"><tr><th>Device</th><th>Model</th><th>IP</th><th>Connected to</th></tr>
    ${gear.map(g => { const conn = hopThroughPatches(g, hostPort(g), FRONT);
      return `<tr><td>${esc(g.name)}</td><td>${esc(DEVICE_TYPES[g.type].label)}</td><td>${hostAddr(g)}</td>
        <td>${conn ? esc(conn.dev.name) + ' · ' + conn.port : 'not connected'}</td></tr>`; }).join('')}</table>`;
}
function ufReaders() {
  return `<p style="color:#8a9099;font-size:13px">Credentials, schedules and unlock policy are an Access device model that is not built yet — specced in ROADMAP before it ships. The hardware inventory is on the Doors page and is real.</p>`;
}
...
//   1. `show` output is RENDERED FROM THE LIVE MODEL, never from a stored
//      string. If the 3D view disagrees with `show running-config`, one of them
//      is a bug — they cannot drift, because there is only one source.
//   2. Config commands MUTATE that same model. Type `switchport access vlan 20`
//      and the simulation, the cable colours and the reachability matrix all
//      change, because the CLI writes the field the engine reads.
// That bidirectionality is the whole point; a terminal that only prints is a
// mockup with a monospace font.

function iosVendor(dev) {
  const def = DEVICE_TYPES[dev.type] || {};
  if (def.cat && /Cisco|Meraki/.test(def.cat)) return 'cisco';
  if (def.cat && /UniFi/.test(def.cat)) return 'unifi';
  return 'cisco';                    // generic gear speaks IOS here
}
function iosHostname(dev) { return (dev.hostname || dev.name || 'Router').replace(/\s+/g, '-'); }
function iosPrompt(sess) {
  const h = iosHostname(sess.dev);
  switch (sess.mode) {
    case 'user': return `${h}>`;
    case 'priv': return `${h}#`;
    ...
    case 'ospf': return `${h}(config-router)#`;
    case 'dhcp': return `${h}(config-dhcp)#`;
    case 'acl': return `${h}(config-ext-nacl)#`;
    default: return `${h}#`;
  }
}
// IOS accepts any unambiguous prefix. Returns the full word, or an error.
function iosExpand(word, choices) {
  if (!word) return { err: 'incomplete' };
  const exact = choices.find(c => c === word);
  if (exact) return { word: exact };
  const hits = choices.filter(c => c.startsWith(word));
  if (hits.length === 1) return { word: hits[0] };
  if (hits.length === 0) return { err: 'invalid' };
  return { err: 'ambiguous', hits };
}
function iosInvalid(line, badIdx) {
  // IOS points a caret at the first token it could not parse.
  const toks = line.split(/\s+/);
  let col = 0;
  for (let i = 0; i < badIdx && i < toks.length; i++) col += toks[i].length + 1;
  return `${' '.repeat(col)}^\n% Invalid input detected at '^' marker.`;
}

// Interface names. IOS calls them GigabitEthernet0/1 and Vlan20; both the long
// and short forms must work because nobody types the long one.
function iosIfName(dev, iface) {
  if (iface && iface.kind === 'svi') return `Vlan${iface.vlan}`;
  const p = iface && iface.port !== undefined ? iface.port : iface;
  return `GigabitEthernet0/${p}`;
}
...
// `show running-config`, generated from the model every time it is asked for.
function iosRunningConfig(dev) {
  const def = DEVICE_TYPES[dev.type] || {};
  const L = ['Building configuration...', '', `version 15.2`, 'service timestamps debug datetime msec',
    '!', `hostname ${iosHostname(dev)}`, '!'];
  // VLAN database
  const db = vlanDb(dev);
  ...
  // DHCP
  if (dev.dhcp && dev.dhcp.enabled) {
    for (const r of dev.dhcp.excluded || []) {
      L.push(`ip dhcp excluded-address ${r.from}${r.to && r.to !== r.from ? ' ' + r.to : ''}`);
    }
    for (const p of dhcpPools(dev)) {
      L.push(`ip dhcp pool ${p.name}`, ` network ${ipStr(p.net.network)} ${ipStr(p.net.mask)}`);
      if (p.defaultRouter) L.push(` default-router ${p.defaultRouter}`);
      if ((p.dns || []).length) L.push(` dns-server ${[].concat(p.dns).join(' ')}`);
      if (p.domain) L.push(` domain-name ${p.domain}`);
      if (p.tftp) L.push(` option 150 ip ${p.tftp}`);
      const l = p.lease === 'infinite' ? ' lease infinite'
        : ` lease ${(p.lease && p.lease.days) || 0} ${(p.lease && p.lease.hours) || 0} ${(p.lease && p.lease.minutes) || 0}`;
      L.push(l);
      for (const r of p.reservations || []) L.push(` ! reservation ${r.mac} ${r.ip}`);
    }
    L.push('!');
  }
  // Interfaces — physical then SVIs, the order IOS prints them.
  const svis = Object.keys(dev.svi || {}).map(Number).sort((a, b) => a - b);
  for (let p = 1; p <= (def.ports || 0); p++) {
    if (portRole(def, p) === 'PWR') continue;
    const cfg = portCfgOf(dev, p);
    const body = [];
    if (cfg.description) body.push(` description ${cfg.description}`);
    if (cfg.ip) { const ip = parseIp(cfg.ip); if (ip) body.push(` ip address ${ipStr(ip.int)} ${ipStr(ip.mask)}`); }
    else if (netClass(dev) === 'switch' || cfg.mode || cfg.vlan) {
      if (portMode(dev, p) === 'trunk') {
        body.push(' switchport mode trunk');
        const nat = nativeVlanOf(dev, p);
        if (nat !== DEFAULT_VLAN) body.push(` switchport trunk native vlan ${nat}`);
    ...
    if (cfg.duplex) body.push(` duplex ${cfg.duplex}`);
    if (cfg.portfast) body.push(' spanning-tree portfast');
    const ap = (dev.aclApply || {})[p];
    if (ap) for (const [dir, id] of Object.entries(ap)) body.push(` ip access-group ${id} ${dir}`);
    const hp = (dev.helpers || {})[`Port ${p}`];
    if (hp) for (const h of [].concat(hp)) body.push(` ip helper-address ${h}`);
    if (cfg.shutdown) body.push(' shutdown');
    if (!body.length) continue;
    L.push(`interface ${iosIfName(dev, p)}`, ...body, '!');
  }
  for (const v of svis) {
    const ip = parseIp(dev.svi[v]);
    const body = [];
    if (ip) body.push(` ip address ${ipStr(ip.int)} ${ipStr(ip.mask)}`);
    const ap = (dev.aclApply || {})[`Vlan${v}`];
    if (ap) for (const [dir, id] of Object.entries(ap)) body.push(` ip access-group ${id} ${dir}`);
    const hp = (dev.helpers || {})[`Vlan${v}`];
    if (hp) for (const h of [].concat(hp)) body.push(` ip helper-address ${h}`);
    L.push(`interface Vlan${v}`, ...body, '!');
  }
  // Routing
  if (ospfEnabled(dev)) {
    L.push(`router ospf 1`);
    if (dev.ospf.routerId) L.push(` router-id ${dev.ospf.routerId}`);
    if (dev.ospf.refBw && dev.ospf.refBw !== OSPF_REF_BW_MBPS) L.push(` auto-cost reference-bandwidth ${dev.ospf.refBw}`);
    for (const i of deviceInterfaces(dev)) {
      L.push(` network ${ipStr(i.ip.network)} ${ipStr(~i.ip.mask >>> 0)} area ${ospfAreaOf(dev, i)}`);
      if (ospfPassive(dev, i)) L.push(` passive-interface ${ifName(i)}`);
    }
    L.push('!');
  }
  for (const r of dev.routes || []) {
    const d = parseIp(r.prefix);
    if (!d) continue;
    L.push(`ip route ${ipStr(d.network)} ${ipStr(d.mask)} ${r.via}${r.ad !== undefined ? ' ' + r.ad : ''}`);
  }
  // NAT
  if (dev.nat && dev.nat.enabled) {
    for (const st of dev.nat.statics || []) {
      L.push(st.proto
        ? `ip nat inside source static ${st.proto} ${st.local} ${st.localPort || ''} ${st.global} ${st.globalPort || ''}`.replace(/\s+/g, ' ')
        : `ip nat inside source static ${st.local} ${st.global}`);
    }
    if (dev.nat.insideGlobal) {
      L.push(`ip nat inside source list ${dev.nat.aclId || 1} interface <outside>${dev.nat.overload ? ' overload' : ''}`);
    }
  }
  // ACLs
  const acl = aclToText(dev.acls);
  if (acl) L.push(acl, '!');
  L.push('end');
  return L.join('\n');
}
...
function iosShow(dev, args) {
  const j = args.join(' ');
  const has = (...pats) => pats.some(p => j.startsWith(p));
  if (!args.length) return { err: 'incomplete' };
  if (has('running-config', 'run', 'ru')) return { out: iosRunningConfig(dev) };
  if (has('startup-config', 'start')) return { out: iosRunningConfig(dev) };
  if (has('version', 'ver')) return { out: iosShowVersion(dev) };
  if (has('ip route', 'ip ro')) return { out: showIpRoute(dev) };
  if (has('ip interface brief', 'ip int b', 'ip int brief', 'ip interface b')) return { out: iosShowIpIntBrief(dev) };
  if (has('interfaces status', 'int status', 'interface status')) return { out: showInterfacesStatus(dev) };
  if (has('interfaces', 'int')) return { out: showInterfacesStatus(dev) };
  if (has('vlan brief', 'vlan')) return { out: iosShowVlan(dev) };
  if (has('mac address-table', 'mac-address-table', 'mac')) return { out: iosShowMac(dev) };
  if (has('spanning-tree', 'span')) return { out: iosShowSpanningTree(dev) };
  if (has('ip ospf neighbor', 'ip ospf nei')) return { out: showIpOspfNeighbor(dev) };
  if (has('ip ospf')) return { out: showIpOspfNeighbor(dev) };
  if (has('ip nat translations', 'ip nat')) return { out: showIpNat(dev) };
  if (has('access-lists', 'access-list', 'acc')) return { out: aclShow(dev) || 'No access lists configured' };
  if (has('ip dhcp binding')) return { out: iosShowDhcpBinding(dev) };
  if (has('ip dhcp pool', 'ip dhcp')) return { out: iosShowDhcpPool(dev) };
  if (has('arp')) return { out: iosShowArp(dev) };
  if (has('ip')) return { err: 'incomplete' };
  return { err: 'invalid', idx: 1 };
}
function iosShowVersion(dev) {
  const def = DEVICE_TYPES[dev.type] || {};
  return [`Cisco IOS Software, Version 15.2(7)E, RELEASE SOFTWARE`,
    `${iosHostname(dev)} uptime is ${fmtDur(simNow() - (dev._boot || (dev._boot = simNow())))}`,
    ``, `Model: ${def.label || dev.type}`,
    `Base ethernet MAC Address       : ${deviceMac(dev)}`,
    `Number of interfaces            : ${def.ports || 0}`].join('\n');
}
function iosShowIpIntBrief(dev) {
  const def = DEVICE_TYPES[dev.type] || {};
  const rows = ['Interface              IP-Address      OK? Method Status                Protocol'];
  const line = (name, ip, up) =>
    `${name.padEnd(22)} ${(ip || 'unassigned').padEnd(15)} YES manual ${(up ? 'up' : 'administratively down').padEnd(21)} ${up ? 'up' : 'down'}`;
  for (let p = 1; p <= (def.ports || 0); p++) {
    if (portRole(def, p) === 'PWR') continue;
    const cfg = portCfgOf(dev, p);
    const l = linkFor(dev.id, p);
    rows.push(line(iosIfName(dev, p), cfg.ip ? parseIp(cfg.ip) && ipStr(parseIp(cfg.ip).int) : null,
      !!(l && l.up)));
  }
  for (const [v, a] of Object.entries(dev.svi || {})) {
    const ip = parseIp(a);
    rows.push(line(`Vlan${v}`, ip ? ipStr(ip.int) : null, true));
  }
  return rows.join('\n');
}
function iosShowVlan(dev) {
  const db = vlanDb(dev);
  const def = DEVICE_TYPES[dev.type] || {};
  const rows = ['VLAN Name                             Status    Ports',
    '---- -------------------------------- --------- -------------------------------'];
  for (const v of [...db.values()].sort((a, b) => a.id - b.id)) {
    const ports = [];
    for (let p = 1; p <= (def.ports || 0); p++) {
      if (portRole(def, p) === 'PWR') continue;
      if (portMode(dev, p) === 'access' && nativeVlanOf(dev, p) === v.id) ports.push(`Gi0/${p}`);
    }
    rows.push(`${String(v.id).padEnd(4)} ${v.name.padEnd(32)} active    ${ports.join(', ')}`);
  }
  return rows.join('\n');
}
function iosShowMac(dev) {
  const e = macEntries(dev.id);
  if (!e.length) return `${iosHostname(dev)}: MAC address table is empty — run a ping to populate it`;
  const rows = ['          Mac Address Table', '-------------------------------------------', '',
    'Vlan    Mac Address       Type        Ports', '----    -----------       --------    -----'];
  for (const [mac, v] of e) rows.push(`${String(v.vlan).padEnd(7)} ${mac.toLowerCase().padEnd(17)} DYNAMIC     Gi0/${v.port}`);
  rows.push(`Total Mac Addresses for this criterion: ${e.length}`);
  return rows.join('\n');
}
function iosShowSpanningTree(dev) {
  const out = [];
  for (const v of [undefined, ...stpVlans()]) {
    const st = stpState(v);
    ...
    if (!root) continue;
    out.push(v === undefined ? 'VLAN0001' : `VLAN${String(v).padStart(4, '0')}`);
    out.push(`  Spanning tree enabled protocol ${stpMode(dev) === 'rapid-pvst' ? 'rstp' : 'ieee'}`);
    out.push(`  Root ID    Priority    ${bridgePriority(root, v)}`);
    out.push(`             Address     ${deviceMac(root).toLowerCase()}`);
    out.push(`             Cost        ${st.cost.get(dev.id) ?? 0}`);
    out.push(`  Bridge ID  Priority    ${bridgePriority(dev, v)}`);
    out.push(`             Address     ${deviceMac(dev).toLowerCase()}`);
    out.push('');
    out.push('Interface           Role Sts Cost      Prio.Nbr Type');
    out.push('------------------- ---- --- --------- -------- ----');
    for (const [k, role] of st.roles) {
      if (!k.startsWith(dev.id + ':')) continue;
      const port = k.split(':')[1];
      const ps = stpPortState(dev.id, port, v);
      const sts = ps.state === 'forwarding' ? 'FWD' : ps.state === 'blocking' || ps.state === 'discarding' ? 'BLK' :
        ps.state === 'learning' ? 'LRN' : 'LIS';
      out.push(`${('Gi0/' + port).padEnd(19)} ${role.slice(0, 4).padEnd(4)} ${sts} ${String(stpCost(linkSpeed({ a: { deviceId: dev.id, port: +port }, b: { deviceId: dev.id, port: +port } }) || 1).toString()).padEnd(9)} 128.${port}    P2p`);
    }
    out.push('');
  }
  return out.length ? out.join('\n') : 'No spanning tree instances — this device has no switch-to-switch links';
}
function iosShowDhcpBinding(dev) {
  const b = [...(_dhcpBindings ? _dhcpBindings.values() : [])].filter(x => x.serverId === dev.id);
  if (!b.length) return 'No DHCP bindings';
  const rows = ['IP address       Client-ID/              Lease expiration        Type',
    '                 Hardware address'];
  for (const x of b) rows.push(`${x.ip.padEnd(16)} ${x.clientId.padEnd(23)} ${fmtDur(x.expiresAt - simNow()).padEnd(23)} ${x.type}`);
  return rows.join('\n');
}
function iosShowDhcpPool(dev) {
  const pools = dhcpPools(dev);
  if (!pools.length) return 'No DHCP pools configured';
  return pools.map(p => {
    const leased = [...(_dhcpBindings ? _dhcpBindings.values() : [])].filter(x => x.serverId === dev.id && x.poolName === p.name).length;
    return [`Pool ${p.name} :`, ` Utilization mark (high/low)    : 100 / 0`,
      ` Subnet size (first/next)       : 0 / 0`,
      ` Total addresses                : ${p.end - p.start + 1}`,
      ` Leased addresses               : ${leased}`,
      ` Range                          : ${ipStr(p.start)} - ${ipStr(p.end)}`].join('\n');
  }).join('\n');
}
function iosShowArp(dev) {
  const c = arpCaches.get(dev.id);
  if (!c || !c.size) return 'ARP cache is empty';
  const rows = ['Protocol  Address          Age (min)  Hardware Addr   Type   Interface'];
  for (const [ip, mac] of c) rows.push(`Internet  ${ip.padEnd(16)} 0          ${mac.toLowerCase()}  ARPA`);
  return rows.join('\n');
}


// ---- config-mode command handlers, each MUTATES the model ----
// Every one of these writes a field the engine reads, then flushes the caches
// so the change takes effect on the next show/ping. That flush is what makes the
// terminal and the 3D view the same object rather than two views that drift.
function iosApply(sess, line) {
  const dev = sess.dev;
  const toks = line.trim().split(/\s+/);
  const no = toks[0] === 'no';
  const t = no ? toks.slice(1) : toks;
  const head = (t[0] || '').toLowerCase();
  const commit = () => { flushL2Tables(); dhcpClearBindings(); resolveDhcp(); refreshAfterCli(dev); };

  // --- global config mode ---
  if (sess.mode === 'config') {
    if (match(head, 'hostname')) { dev.hostname = t[1] || dev.hostname; updateDeviceFaceplate(dev.id); return {}; }
    if (match(head, 'interface')) {
      const iface = iosParseIf(dev, t.slice(1));
      if (!iface) return { err: 'invalid', idx: 1 };
      sess.mode = 'if'; sess.iface = iface; return {};
    }
    if (match(head, 'vlan')) {
      const id = +t[1];
    ...
    if (head === 'ip' && match((t[1] || '').toLowerCase(), 'route')) {
      const net = parseIp(t[2]), mask = t[3], via = t[4];
      if (!net || !mask || !via) return { err: 'incomplete' };
      const cidr = validMask(mask);
      if (cidr === null || cidr === undefined) return { err: 'invalid', idx: 3 };
      if (!parseIp(via)) return { err: 'invalid', idx: 4 };
      dev.routes = dev.routes || [];
      const prefix = `${ipStr(net.network)}/${cidr}`;
      dev.routes = dev.routes.filter(r => r.prefix !== prefix || (t[5] && +r.ad !== +t[5]));
      if (!no) dev.routes.push({ prefix, via, ...(t[5] ? { ad: +t[5] } : {}) });
      commit(); return {};
    }
    if (head === 'ip' && match((t[1] || '').toLowerCase(), 'access-list')) {
      const kind = (t[2] || '').toLowerCase();
      if (kind !== 'standard' && kind !== 'extended') return { err: 'invalid', idx: 2 };
      sess.mode = 'acl'; sess.aclName = t[3]; sess.aclKind = kind;
      dev.acls = dev.acls || [];
      if (!dev.acls.some(a => String(a.id) === t[3])) dev.acls.push({ id: t[3], kind, named: true, rules: [] });
      return {};
    }
    if (head === 'access-list') {
      const res = parseAclText(line);
      if (res.errors.length) return { err: 'invalid', idx: 1 };
      dev.acls = mergeAcls(dev.acls || [], res.acls); commit(); return {};
    }
    if (match(head, 'router')) {
      if (match((t[1] || '').toLowerCase(), 'ospf')) { dev.ospf = dev.ospf || { enabled: true, area: 0 }; dev.ospf.enabled = true; sess.mode = 'ospf'; commit(); return {}; }
      return { err: 'invalid', idx: 1 };
    }
    if (head === 'ip' && match((t[1] || '').toLowerCase(), 'dhcp')) {
      if (match((t[2] || '').toLowerCase(), 'pool')) {
        dev.dhcp = dev.dhcp || { enabled: true, pools: [], excluded: [] };
        dev.dhcp.enabled = true; dev.dhcp.pools = dev.dhcp.pools || [];
        let pool = dev.dhcp.pools.find(p => p.name === t[3]);
        if (!pool) { pool = { name: t[3], network: '', reservations: [] }; dev.dhcp.pools.push(pool); }
        sess.mode = 'dhcp'; sess.pool = pool; commit(); return {};
      }
      if (match((t[2] || '').toLowerCase(), 'excluded-address')) {
        dev.dhcp = dev.dhcp || { enabled: true, pools: [], excluded: [] };
        dev.dhcp.excluded = dev.dhcp.excluded || [];
        if (no) dev.dhcp.excluded = dev.dhcp.excluded.filter(e => e.from !== t[3]);
        else dev.dhcp.excluded.push({ from: t[3], to: t[4] || t[3] });
        commit(); return {};
      }
    }
    if (match(head, 'end')) { sess.mode = 'priv'; return {}; }
    if (match(head, 'exit')) { sess.mode = 'priv'; return {}; }
    return { err: 'invalid', idx: 0 };
  }

  // --- interface config mode ---
  if (sess.mode === 'if') {
    const port = sess.iface.port, isSvi = sess.iface.kind === 'svi';
    const ensureCfg = () => { dev.portCfg = dev.portCfg || {}; dev.portCfg[port] = dev.portCfg[port] || {}; return dev.portCfg[port]; };
    if (match(head, 'exit')) { sess.mode = 'config'; return {}; }
    if (match(head, 'shutdown')) {
      if (isSvi) return { err: 'invalid', idx: 0 };
      ensureCfg().shutdown = !no; commit(); return {};
    }
    ...
    if (head === 'ip' && match((t[1] || '').toLowerCase(), 'helper-address')) {
      dev.helpers = dev.helpers || {};
      const key = isSvi ? `Vlan${sess.iface.vlan}` : `Port ${port}`;
      const list = [].concat(dev.helpers[key] || []);
      if (no) dev.helpers[key] = list.filter(h => h !== t[2]);
      else { list.push(t[2]); dev.helpers[key] = list; }
      commit(); return {};
    }
    if (head === 'ip' && match((t[1] || '').toLowerCase(), 'access-group')) {
      dev.aclApply = dev.aclApply || {};
      const key = isSvi ? `Vlan${sess.iface.vlan}` : port;
      dev.aclApply[key] = dev.aclApply[key] || {};
      const dir = (t[3] || '').toLowerCase();
      if (no) delete dev.aclApply[key][dir]; else dev.aclApply[key][dir] = t[2];
      commit(); return {};
    }
    if (match(head, 'switchport')) {
      const c = ensureCfg();
      ...
      if (match(sub, 'trunk')) {
        ...
        if (match(w, 'native')) {
          const nv = +t[4];
          if (!vlanIdOk(nv)) return { err: 'invalid', idx: 4 };
          c.native = t[4]; c.mode = 'trunk'; commit(); return {};
        }
        if (match(w, 'allowed')) {
          const listCheck = (t[t.length - 1] || '').replace(/[^\d,\-]/g, '');
          for (const part of listCheck.split(',').filter(Boolean)) {
            for (const n of part.split('-').filter(Boolean)) {
              if (!vlanIdOk(+n)) return { err: 'invalid', idx: t.length - 1 };
            }
          }
          c.mode = 'trunk';
          const vlans = t.slice(t.indexOf(t.find(x => /^\d/.test(x))) ).join('').replace(/[^\d,]/g, '');
          const list = (t[t.length - 1] || '').replace(/[^\d,]/g, '');
          if (t.includes('add')) c.tagged = [...new Set([...(c.tagged || '').split(',').filter(Boolean), ...list.split(',')])].join(',');
          else c.tagged = list;
          commit(); return {};
        }
      }
    ...
    if (match(head, 'speed')) {
      ...
      if (!w) return { err: 'incomplete' };
      if (w !== 'auto' && !ETH_SPEEDS.includes(+w)) return { err: 'invalid', idx: 1 };
      ensureCfg().speed = w === 'auto' ? '' : +w / 1000;
      commit(); return {};
    }
    if (match(head, 'duplex')) {
      const w = (t[1] || '').toLowerCase();
      if (!w) return { err: 'incomplete' };
      if (!['auto', 'full', 'half'].includes(w)) return { err: 'invalid', idx: 1 };
      ensureCfg().duplex = w === 'auto' ? '' : w;
      commit(); return {};
    }
  ...
  if (sess.mode === 'ospf') {
    if (match(head, 'network')) {
      // network A.B.C.D wildcard area N — the area is per interface in our model
      const net = parseIp(t[1]); const area = +t[t.length - 1];
      if (net) { dev.ospf.areas = dev.ospf.areas || {};
        for (const i of deviceInterfaces(dev)) if (ipInSubnet(i.ip.int, { ...net, mask: ~(parseIp(t[2]) || { int: 0 }).int >>> 0, network: net.int & (~(parseIp(t[2]) || { int: 0 }).int >>> 0) })) dev.ospf.areas[ifKey(i)] = area; }
      commit(); return {};
    }
    if (match(head, 'router-id')) { dev.ospf.routerId = t[1]; commit(); return {}; }
    if (head === 'auto-cost') { dev.ospf.refBw = +t[t.length - 1] || OSPF_REF_BW_MBPS; commit(); return {}; }
    if (match(head, 'passive-interface')) {
      const iface = iosParseIf(dev, t.slice(1));
      dev.ospf.passive = dev.ospf.passive || [];
      if (iface) { const k = iface.kind === 'svi' ? `Vlan${iface.vlan}` : iface.port;
        if (no) dev.ospf.passive = dev.ospf.passive.filter(x => x !== k); else dev.ospf.passive.push(k); }
      commit(); return {};
    }
    if (match(head, 'exit')) { sess.mode = 'config'; return {}; }
    return { err: 'invalid', idx: 0 };
  ...
  if (sess.mode === 'dhcp') {
    const p = sess.pool;
    if (match(head, 'network')) { const net = parseIp(t[1]); if (net) p.network = `${ipStr(net.int)}/${t[2] ? maskToCidr(t[2]) : net.cidr}`; commit(); return {}; }
    if (match(head, 'default-router')) { p.defaultRouter = t[1]; commit(); return {}; }
    if (match(head, 'dns-server')) { p.dns = t.slice(1); commit(); return {}; }
    if (match(head, 'domain-name')) { p.domain = t[1]; commit(); return {}; }
    if (match(head, 'lease')) { p.lease = t[1] === 'infinite' ? 'infinite' : { days: +t[1] || 0, hours: +t[2] || 0, minutes: +t[3] || 0 }; commit(); return {}; }
    if (match(head, 'exit')) { sess.mode = 'config'; return {}; }
    return { err: 'invalid', idx: 0 };
  }
  if (sess.mode === 'acl') {
    if (match(head, 'exit')) { sess.mode = 'config'; return {}; }
    // reuse the ACL text parser for a single named-list body line
    const full = `ip access-list ${sess.aclKind} ${sess.aclName}\n ${line.trim()}`;
    const res = parseAclText(full);
    if (res.errors.length) return { err: 'invalid', idx: 0 };
    dev.acls = mergeAcls(dev.acls || [], res.acls); commit(); return {};
  }
  return { err: 'invalid', idx: 0 };
}
function match(word, full) { return word && full.startsWith(word); }
function maskToCidr(mask) { const p = parseIp(mask + '/32'); if (!p) return 24; let m = p.int, c = 0; while (m & 0x80000000) { c++; m = (m << 1) >>> 0; } return c; }
function mergeAcls(existing, incoming) {
  const byId = new Map(existing.map(a => [String(a.id), a]));
  for (const a of incoming) {
    const cur = byId.get(String(a.id));
    if (cur) { cur.rules = [...cur.rules, ...a.rules]; cur.named = cur.named || a.named; cur.kind = cur.kind || a.kind; }
    else byId.set(String(a.id), a);
  }
  return [...byId.values()];
}
// After a CLI edit, rebuild whatever the change could have moved in 3D.
function refreshAfterCli(dev) {
  try { if (typeof rebuildDevice === 'function') rebuildDevice(dev.id);
    for (const c of state.cables) rebuildCable && rebuildCable(c.id);
    if (typeof refreshSimPickers === 'function') refreshSimPickers();
  } catch (e) {}
}

// ---- the session: one line in, output + new prompt out ----
function iosNewSession(dev) { return { dev, mode: 'user', iface: null }; }
function iosExec(sess, raw) {
  ...
  // navigation available in exec modes
  if (sess.mode === 'user') {
  ...
  if (sess.mode === 'priv') {
    if (match(head, 'disable')) { sess.mode = 'user'; return { out: '' }; }
    if (match(head, 'exit') || match(head, 'logout')) return { out: '', close: true };
    ...
    if (head === 'clear') {
      ...
      if (toks.join(' ').includes('mac')) { macTables.clear(); return { out: '' }; }
  ...
  return iosResult(r, line);
}
function iosResult(r, line) {
...
// Can THIS device reach that address? Uses the real engine, not a proxy for it.
// The first version asked "does a routing table entry exist", which is not the
// same question: a switch has no routing table at all, so pinging from a switch
// always failed even when the two boxes were plainly on the same wire. Real
// reachability is an L2 walk when the target is on our subnet, and a forwarding
// path when it is not.
function deviceReach(srcDev, dstInt) {
  const owner = deviceHoldingAddr(dstInt);
  if (!owner) return { ok: false, why: `nothing answers at ${ipStr(dstInt)}` };
  if (owner.dev.id === srcDev.id) return { ok: true };            // our own address
  const ifaces = deviceInterfaces(srcDev);
  if (!ifaces.length) return { ok: false, why: `${srcDev.name} has no IP address configured` };
  // Prefer an interface whose subnet contains the target — that is the one a
  // real box would source from.
  const local = ifaces.find(i => ipInSubnet(dstInt, i.ip));
  if (local) {
    // Same subnet: it is an ARP-and-switch question. Walk L2 from every data
    // port and see whether the owning device is reached.
    const def = DEVICE_TYPES[srcDev.type] || {};
    const vlan = local.kind === 'svi' ? local.vlan : undefined;
    for (let p = 1; p <= (def.ports || 1); p++) {
      if (portRole(def, p) === 'PWR') continue;
    ...
    return { ok: false, why: `${ipStr(dstInt)} is on this subnet but no L2 path reaches it` };
  }
  // Off-subnet: this device must route, or hand off to a gateway.
  if (netClass(srcDev) === 'router') {
    const f = forwardPath(srcDev.id, dstInt);
  ...
  if (!holder) return { ok: false, why: `default gateway ${gwStr} does not answer` };
  const f = forwardPath(holder.dev.id, dstInt);
  return f.error ? { ok: false, why: f.error } : { ok: true };
}
...
// one of the most common real faults there is. So a ping is two PDU walks, and
// the reply is sent from whichever NIC received the request.
function pduPing(srcDev, dstDev, spec = {}) {
  const dstNic = hostNics(dstDev).find(n => nicIp(dstDev, n.port));
  if (!dstNic) return { request: null, reply: null, ok: false, why: noAddressReason(dstDev) };
  const dstIp = nicIp(dstDev, dstNic.port);
  const request = pduSend(srcDev, dstIp.int, { ...spec, label: 'echo request' });
  if (!request.delivered) {
    return { request, reply: null, ok: false, why: request.dropped ? request.dropped.why : 'request never arrived' };
  }
  // The reply's source is the address the request was sent to, and it goes back
  // to the request's source address — not necessarily by the same path.
  const srcNic = hostEgressNic(srcDev, dstIp.int);
  const backTo = nicIp(srcDev, srcNic.port);
  const reply = pduSend(dstDev, backTo.int, {
    ...spec, icmpType: spec.proto && spec.proto !== 'icmp' ? null : 'echo-reply',
    flags: spec.proto === 'tcp' ? 'SYN,ACK' : undefined, label: 'echo reply' });
  return { request, reply, ok: !!reply.delivered,
    why: reply.delivered ? null
      : `the request arrived but the reply could not get back — ${reply.dropped ? reply.dropped.why : 'unknown'}` };
}

// ---- PDU animation in 3D ----
// The trace is a list of device decisions; this turns it into a marker that
// travels the actual cables between those devices, so watching it and reading
// the trace are the same thing.
let _pduMarker = null, _pduAnim = null;
function pduClearMarker() {
  if (_pduMarker) { scene.remove(_pduMarker); _pduMarker.geometry.dispose(); _pduMarker.material.dispose(); _pduMarker = null; }
  _pduAnim = null;
}
function pduShowAt(trace, idx) {
  if (!trace || !trace.length) return;
  const t = trace[Math.max(0, Math.min(trace.length - 1, idx))];
  const g = deviceGroups.get(t.devId);
  if (!g) return;
  g.updateMatrixWorld(true);
  const pos = g.getWorldPosition(new THREE.Vector3());
  if (!_pduMarker) {
    _pduMarker = new THREE.Mesh(new THREE.SphereGeometry(1.6, 16, 16),
      new THREE.MeshBasicMaterial({ color: 0xffd23e, transparent: true, opacity: 0.9 }));
    _pduMarker.renderOrder = 999;
    scene.add(_pduMarker);
  }
  const col = t.action === 'drop' ? 0xff453a : (t.action === 'deliver' ? 0x30d158 : 0xffd23e);
  _pduMarker.material.color.setHex(col);
  _pduMarker.position.copy(pos).add(new THREE.Vector3(0, 3, 0));
}
// Realtime mode: play the trace at a watchable pace instead of stepping it.
function pduPlay(trace, onStep) {
  pduStop();
  let i = 0;
  _pduAnim = setInterval(() => {
    i++;
    if (i > trace.length) { pduStop(); return; }
    pduShowAt(trace, i - 1);
    if (onStep) onStep(i);
  }, 700);
}
function pduStop() { if (_pduAnim) { clearInterval(_pduAnim); _pduAnim = null; } }
...
let _pduCur = null, _pduAt = 0, _pduLeg = 'request', _pduFilter = 'all';
function runPdu() {
  ...
  aiPrint(`<div class="row" style="gap:4px;flex-wrap:wrap">
      <select id="pdu-a" style="width:100px">${opts}</select><span class="sim-arrow">→</span>
      <select id="pdu-b" style="width:100px">${opts}</select>
      <select id="pdu-proto" style="width:58px">
  ...
  document.getElementById('pdu-send').onclick = () => {
    ...
    const spec = proto === 'icmp' ? { proto: 'icmp', icmpType: 'echo' }
      : { proto, srcPort: 49152, dstPort: raw ? (aclPortNum(raw, proto) || +raw || 80) : 80,
          flags: proto === 'tcp' ? 'SYN' : null };
    _pduCur = pduPing(src, dst, spec);
    _pduLeg = 'request';
...
function drawPduTrace(target) {
  ...
  const full = sim.trace;
  const shown = full.slice(0, _pduAt);
  const visible = _pduFilter === 'all' ? shown : shown.filter(t => t.pdu && t.pdu.proto === _pduFilter);
  let html = `<div class="row" style="gap:4px;margin:8px 0;flex-wrap:wrap">
      <button id="pdu-leg-req" class="${_pduLeg === 'request' ? 'active' : ''}">Request</button>
      <button id="pdu-leg-rep" class="${_pduLeg === 'reply' ? 'active' : ''}" ${_pduCur.reply ? '' : 'disabled'}>Reply</button>
      <select id="pdu-filter" style="width:74px">
        ${['all', 'arp', 'icmp', 'tcp', 'udp'].map(f => `<option value="${f}" ${_pduFilter === f ? 'selected' : ''}>${f}</option>`).join('')}
      </select></div>
    <div class="row" style="gap:4px;margin-bottom:6px;flex-wrap:wrap">
      <button id="pdu-back">← Back</button><button id="pdu-fwd">Step →</button>
      <button id="pdu-play">Play</button><button id="pdu-stop">Stop</button>
      <button id="pdu-all">End</button>
      <span class="ai-meta">${_pduAt} / ${full.length}</span></div>`;
  ...
  visible.forEach((t) => {
    ...
    html += `<tr><td>${full.indexOf(t) + 1}</td><td>${esc(d ? d.name : '?')}</td>` +
      `<td>${t.egress ? `${t.iface}→${t.egress}` : t.iface}</td>` +
  ...
  if (_pduAt === full.length) {
    html += sim.delivered
      ? `<p class="ai-good">${_pduLeg === 'reply' ? 'Reply' : 'Request'} delivered in ${full.length} steps.</p>`
      : `<p class="ai-bad">Dropped — ${esc(sim.dropped ? sim.dropped.why : 'unknown')}</p>`;
    // The round trip is the real verdict: a request that arrives and a reply
    // that cannot get home is a failed ping, and says so.
    if (_pduLeg === 'request' && _pduCur.request && _pduCur.request.delivered) {
      html += _pduCur.ok
        ? `<p class="ai-good">Round trip complete — switch to Reply to watch the answer come back.</p>`
        : `<p class="ai-bad">${esc(_pduCur.why || 'the reply did not get back')}</p>`;
    }
  }
  out.innerHTML = html;
...
// so closing the console and looking at the 3D view or running a ping shows the
// change immediately.
let _termSess = null, _termHist = [], _termHistAt = 0;
...
(function initTerm() {
  ...
  const out = document.getElementById('term-out');
  const inp = document.getElementById('term-in');
  document.getElementById('term-close').onclick = () => { box.classList.add('hidden'); _termSess = null; };
  inp.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      ...
      if (_termHistAt < _termHist.length) { _termHistAt++; inp.value = _termHist[_termHistAt] || ''; }
      e.preventDefault();
    }
  });
  // clicking anywhere in the console focuses the input, like a real terminal
...
const THEMES = {
  studio: {
    ...
    sun: [0xfff4e2, 2.6], rim: 0.5, exposure: 0.95,
...
//////////////////// Enterprise data centre — THE test environment ////////////////////
// A real facility, built to function, not to look busy. Every VLAN, route, ACL,
// DHCP scope, camera and door here is wired the way it would be on site, so the
// simulation can be tested against something that has to actually work.
//
// Scale is a parameter. It is deliberately modest until the instanced render
// layer lands (see ARCHITECTURE DECISION in ROADMAP) — the design does not
// change when it is cranked up, only the row/rack counts.
//
// VLAN plan, the way a real DC segments:
//   10 MGMT-OOB   out-of-band management, isolated, reachable only from jump
//   20 PROD       production server front-end
//   30 DMZ        internet-facing
//   40 STORAGE    east-west storage fabric, no gateway off-net
//   50 CAMERA     UniFi Protect cameras + NVR, isolated by ACL
//   60 ACCESS     door controllers/readers, isolated by ACL
//   70 CORP       staff / NOC workstations
// Subnet SIZE is a real design decision, not decoration: a /24 holds 254 hosts,
// and a hall with 100 racks holds thousands of servers. Sizing every VLAN /24
// silently produced addresses like 10.10.20.650 — invalid, and hundreds of hosts
// ended up with no address at all. The big host VLANs are /16s, the way a real
// data centre sizes them; the small service VLANs stay /24.
const DC_VLANS = [
  { id: 10, name: 'MGMT-OOB', net: '10.10',    cidr: 16, base: '10.10.0.0' },
  { id: 20, name: 'PROD',     net: '10.20',    cidr: 16, base: '10.20.0.0' },
  { id: 30, name: 'DMZ',      net: '10.30',    cidr: 16, base: '10.30.0.0' },
  { id: 40, name: 'STORAGE',  net: '10.40',    cidr: 16, base: '10.40.0.0' },
  { id: 50, name: 'CAMERA',   net: '10.50',    cidr: 16, base: '10.50.0.0' },
  { id: 60, name: 'ACCESS',   net: '10.60.0',  cidr: 24, base: '10.60.0.0' },
  { id: 70, name: 'CORP',     net: '10.70.0',  cidr: 24, base: '10.70.0.0' },
];
// nth usable host address in a VLAN, honouring its real prefix length
// The prefix length is part of the address. A bare IP parses as /24, so a host
// in a /16 VLAN would carry the wrong mask and never match its own gateway —
// exactly the fault a mistyped mask causes on a real machine.
function dcHost(v, n) {
  const a = v.cidr === 24 ? `${v.net}.${10 + (n % 240)}`
                          : `${v.net}.${1 + Math.floor(n / 250)}.${10 + (n % 250)}`;
  return `${a}/${v.cidr}`;
}
function dcGw(v) { return v.cidr === 24 ? `${v.net}.1` : `${v.net}.0.1`; }

function buildDataCentre(opts = {}) {
  const ROWS = opts.rows || 2, PER_ROW = opts.racksPerRow || 6;
  clearScene();
  const D = [], C = [];
  const put = (o) => { state.devices.push(o); buildDeviceGroup(o); D.push(o); return o; };
  const rack = (x, z, name, units = 42) => {
    const r = { id: uid(), x, z, y0: 0, rotY: 0, units, mount: 'floor', name };
    state.racks.push(r); buildRackGroup(r); return r;
  };
  ...
  const wall = (x1, z1, x2, z2, material, openings) => {
    const w = { id: uid(), x1, z1, x2, z2, h: WALL_H, y0: 0, material, openings: openings || [] };
    state.walls.push(w); buildWall(w); return w;
  };
  const slab = (x1, z1, x2, z2, material, y) => {
    const sl = { id: uid(), x1, z1, x2, z2, y: y || 0, material };
    state.slabs.push(sl); buildSlab(sl); return sl;
  };
  const svi = {}, pools = [];
  for (const v of DC_VLANS) {
    svi[v.id] = `${dcGw(v)}/${v.cidr}`;
    // STORAGE is deliberately static-only — a storage fabric does not DHCP.
    if (v.id === 40) continue;
    const lo = v.cidr === 24 ? `${v.net}.150` : `${v.net}.200.10`;
    const hi = v.cidr === 24 ? `${v.net}.240` : `${v.net}.250.250`;
    pools.push({ name: v.name, network: `${v.base}/${v.cidr}`, poolStart: lo, poolEnd: hi,
      lease: { days: 1 }, defaultRouter: dcGw(v), dns: ['10.20.0.10'], domain: 'dc.local', reservations: [] });
  }
  const V = Object.fromEntries(DC_VLANS.map(v => [v.id, v]));

  // ---- the building: data hall, MDF, UPS room, and a mantrap vestibule ----
  const HALL_X = ROWS * 130 + 120, HALL_Z = PER_ROW * 46 + 120;
  slab(-HALL_X/2, -HALL_Z/2, HALL_X/2, HALL_Z/2, 'concrete');
  // A data hall has a real deck overhead with continuous fixtures above the
  // aisles. Without one the hall was lit by sky and sun only, which is exactly
  // why every interior view measured dark and colourless -- interior light in
  // this app hangs off the ceiling slab.
  slab(-HALL_X/2, -HALL_Z/2, HALL_X/2, HALL_Z/2, 'concrete', WALL_H);
  // perimeter, with the only way in through the vestibule
  wall(-HALL_X/2, -HALL_Z/2,  HALL_X/2, -HALL_Z/2, 'concrete');
  wall(-HALL_X/2,  HALL_Z/2,  HALL_X/2,  HALL_Z/2, 'concrete');
  wall(-HALL_X/2, -HALL_Z/2, -HALL_X/2,  HALL_Z/2, 'concrete');
  wall( HALL_X/2, -HALL_Z/2,  HALL_X/2,  HALL_Z/2, 'concrete');
  // MANTRAP: two doors in series, an airlock — you badge in, the outer door must
  // close before the inner opens. Both leaves are real openings.
  // Both leaves share an interlock group, which is what actually makes it a
  // mantrap: the engine refuses to open one while the other is off its seat.
  // Both are locked, so they only release when a badge is presented.
  const vestZ = -HALL_Z/2;
  wall(-60, vestZ - 90,  60, vestZ - 90, 'concrete',
    [{ id: uid(), kind: 'door', at: 0.5, label: 'Vestibule outer door',
       locked: true, interlock: 'mantrap-1', reader: 'RDR-OUTER' }]);
  wall(-60, vestZ,       60, vestZ,      'concrete',
    [{ id: uid(), kind: 'door', at: 0.5, label: 'Vestibule inner door',
       locked: true, interlock: 'mantrap-1', reader: 'RDR-INNER' }]);
  wall(-60, vestZ - 90, -60, vestZ,      'concrete');
  wall( 60, vestZ - 90,  60, vestZ,      'concrete');
  slab(-60, vestZ - 90, 60, vestZ, 'vct');

  // ---- WAN: two diverse carriers, each with its own ONT ----
  const isp = field('internet', { x: -HALL_X/2 - 140, z: -HALL_Z/2 - 140, name: 'Carrier-Cloud' });
  const ontA = field('ont', { x: -HALL_X/2 - 70, z: -HALL_Z/2 - 160, name: 'ONT-A',
    circuit: { active: true, provider: 'Carrier A', wanIp: '203.0.113.2/30' } });
  const ontB = field('ont', { x: -HALL_X/2 - 70, z: -HALL_Z/2 - 110, name: 'ONT-B',
    circuit: { active: true, provider: 'Carrier B', wanIp: '198.51.100.2/30' } });
  link(isp, 1, ontA, 1); link(isp, 2, ontB, 1);

  // ---- MDF rack: edge, core, OOB, NVR, access controller ----
  const mdf = rack(-HALL_X/2 + 60, -HALL_Z/2 + 70, 'MDF-01');
  inRack(mdf, 'vcm', 0, { side: 'R' });
  const edgeA = inRack(mdf, 'c_mx105', 41, { name: 'EDGE-A', ip: '10.10.0.2/16',
    portCfg: { 2: { ip: '192.168.255.1/29' } },
    ospf: { enabled: true, area: 0, refBw: 10000 } });
  const edgeB = inRack(mdf, 'c_mx95', 39, { name: 'EDGE-B', ip: '10.10.0.3/16',
    portCfg: { 2: { ip: '192.168.255.2/29' } },
    ospf: { enabled: true, area: 0, refBw: 10000 } });
  link(ontA, 2, edgeA, 1); link(ontB, 2, edgeB, 1);
  ...
  const coreA = inRack(mdf, 'c_c9300l48', 37, { name: 'CORE-A', ip: '10.10.0.4/16', svi: { ...svi },
    ...
    nat: { enabled: true, insideGlobal: '203.0.113.2', overload: true },
    dhcp: { enabled: true, pools, excluded: DC_VLANS.map(v => ({ from: `${v.net}.1`, to: `${v.net}.99` })) } });
  link(edgeA, 2, coreA, 1, 0xf0883e);                       // core <-> edge transit

  // The OOB/services switch carries management, camera, access and corp — so its
  // uplink to the core is a TRUNK. An access port here was the bug that left
  // cameras and the NOC stranded: their VLANs could not reach their gateway.
  const oob = inRack(mdf, 'c_ms120_24p', 35, { name: 'OOB-SW', ip: '10.10.0.5/16',
    vlans: DC_VLANS.map(v => ({ id: v.id, name: v.name })),
    portCfg: Object.fromEntries([...Array(23)].map((_, i) => [i + 1, { vlan: '10' }])) });
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let i = 0; i < PER_ROW; i++) {
      ...
      inRack(r, 'hcm', 41);
      // ToR pair is overkill per rack in a small hall; one ToR + uplink to core
      const tor = inRack(r, 'c_ms250_48', 40, { name: `TOR-${row+1}-${i+1}`, ip: `10.10.200.${1 + tors.length % 250}/16`,
        vlans: DC_VLANS.map(v => ({ id: v.id, name: v.name })),
        portCfg: { 48: { mode: 'trunk', native: '10', tagged: DC_VLANS.map(v=>v.id).join(',') } } });
      tors.push(tor);
      inRack(r, 'hcm', 39);
      // servers: dual-homed (PROD + STORAGE), plus an OOB management NIC
      for (let s = 0; s < 6; s++) {
        const u = 36 - s * 2;
        const n = tors.length * 10 + s;
        const srv = inRack(r, 'server', u, { name: `SRV-${row+1}${i+1}-${s+1}`,
          nics: { 1: { ip: dcHost(V[20], n), gateway: dcGw(V[20]), label: 'prod' },
                  2: { ip: dcHost(V[40], n), label: 'storage' },
                  3: { ip: dcHost(V[10], n), gateway: dcGw(V[10]), label: 'oob' } } });
        // prod + storage land on the ToR, management goes back to the OOB switch
        tor.portCfg[s*2+1] = { vlan: '20' }; tor.portCfg[s*2+2] = { vlan: '40' };
        link(srv, 1, tor, s*2+1, 0x2f81f7);
        link(srv, 2, tor, s*2+2, 0xa371f7);
      }
      ...
      link(cam, 1, tor, 40 + (i % 4), 0xdb61a2);
      tor.portCfg[40 + (i % 4)] = { vlan: '50' };
    }
  }

  // ---- physical security: cameras and readers on the entries ----
  const secCams = [
    ['CAM-VEST-OUT', 0, vestZ - 88], ['CAM-VEST-IN', 0, vestZ + 6],
    ['CAM-HALL-N', -HALL_X/2 + 20, -HALL_Z/2 + 20], ['CAM-HALL-S', HALL_X/2 - 20, HALL_Z/2 - 20],
  ];
  for (const [name, x, z] of secCams) {
    const c = field('u_aipro', { mount: 'wall', x, y: 104, z, rotY: 0, name, ip: 'dhcp' });
    link(c, 1, oob, secCams.indexOf(secCams.find(s=>s[0]===name)) + 1, 0xdb61a2);
  }
  // badge readers: outer mantrap door, inner mantrap door, and the cage
  const readers = [['RDR-OUTER', 0, vestZ - 86], ['RDR-INNER', 0, vestZ + 4], ['RDR-CAGE', -HALL_X/2 + 40, -HALL_Z/2 + 40]];
  readers.forEach(([name, x, z], i) => {
    const rd = field('u_readerg3', { mount: 'wall', x, y: 48, z, rotY: 0, name, ip: 'dhcp' });
    link(rd, 1, oob, 10 + i, 0x3fb950);
    oob.portCfg[10 + i] = { vlan: '60' };
  });
  // cameras on OOB ports land in the camera VLAN, not management
  for (let i = 1; i <= secCams.length; i++) oob.portCfg[i] = { vlan: '50' };

  // ---- NOC ----
  const noc = field('o_ws', { x: HALL_X/2 - 60, z: HALL_Z/2 - 60, name: 'NOC-1', ip: 'dhcp' });
  link(noc, 1, oob, 20, 0x2f81f7);
  oob.portCfg[20] = { vlan: '70' };
  const jump = inRack(mdf, 'server', 27, { name: 'JUMP-01',
    nics: { 1: { ip: '10.70.0.10/24', gateway: '10.70.0.1', label: 'corp' },
            2: { ip: '10.10.0.11/16', gateway: '10.10.0.1', label: 'oob' } } });
  link(jump, 1, oob, 21, 0x2f81f7); oob.portCfg[21] = { vlan: '70' };
  link(jump, 2, oob, 22, 0x3fb950); oob.portCfg[22] = { vlan: '10' };
  link(nvr, 1, oob, 23, 0xdb61a2); oob.portCfg[23] = { vlan: '50' };
  link(acHub, 1, oob, 19, 0x3fb950); oob.portCfg[19] = { vlan: '60' };

  // ---- segmentation: the ACLs that make the zones real ----
  coreA.acls = parseAclText([
    ...
    ' 20 permit ip host 10.10.0.11 any',
    ' 30 permit ip host 10.70.0.10 10.10.0.0 0.0.255.255',
...
function referenceSite() {
  // The reference site IS an enterprise data centre — it is the environment
  // everything gets tested against. Three-tier network (edge / core / aggregation
  // / top-of-rack), seven segmented VLANs, dual carriers, mantrap entry, cameras
  // and badge readers on isolated VLANs.
  return buildDataCentre({ rows: 2, racksPerRow: 6 });
}
...
(function animate() {
  ...
  autoDoorsForWalker();
  animateDoors(dt);
  if (_shadowDirty) { renderer.shadowMap.needsUpdate = true; _shadowDirty = false; }
  if (walkActive) {
