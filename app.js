...
const DC_VLANS = [
  { id: 10, name: 'MGMT-OOB', net: '10.10.10' },
  { id: 20, name: 'PROD',     net: '10.10.20' },
  { id: 30, name: 'DMZ',      net: '10.10.30' },
  { id: 40, name: 'STORAGE',  net: '10.10.40' },
  { id: 50, name: 'CAMERA',   net: '10.10.50' },
  { id: 60, name: 'ACCESS',   net: '10.10.60' },
  { id: 70, name: 'CORP',     net: '10.10.70' },
];
...
function buildDataCentre(opts = {}) {
  ...
  for (const v of DC_VLANS) {
    ...
    if (v.id === 40) continue;
    pools.push({ name: v.name, network: `${v.net}.0/24`, poolStart: `${v.net}.100`, poolEnd: `${v.net}.200`,
      lease: { days: 1 }, defaultRouter: `${v.net}.1`, dns: ['10.10.20.10'], domain: 'dc.local', reservations: [] });
  }
  ...
  const edgeA = inRack(mdf, 'c_mx105', 41, { name: 'EDGE-A', ip: '10.10.10.2',
    portCfg: { 2: { ip: '192.168.255.1/29' } },
    ospf: { enabled: true, area: 0, refBw: 10000 } });
  const edgeB = inRack(mdf, 'c_mx95', 39, { name: 'EDGE-B', ip: '10.10.10.3',
  ...
  const coreA = inRack(mdf, 'u_udmpromax', 37, { name: 'CORE-A', ip: '10.10.10.4', svi: { ...svi },
  ...
  const oob = inRack(mdf, 'c_ms120_24p', 35, { name: 'OOB-SW', ip: '10.10.10.5',
  ...
  const agg = inRack(mdf, 'c_c9300l48', 33, { name: 'AGG-01', ip: '10.10.10.6',
    vlans: DC_VLANS.map(v => ({ id: v.id, name: v.name })) });
  agg.portCfg = { 48: { mode: 'trunk', native: '10', tagged: DC_VLANS.map(v => v.id).join(',') } };
  link(agg, 48, coreA, 4, 0xf0883e);
  ...
  const nvr = inRack(mdf, 'u_unvrpro', 31, { name: 'NVR-01', ip: '10.10.50.10', gateway: '10.10.50.1' });
  const acHub = inRack(mdf, 'u_accesshub', 29, { name: 'ACCESS-HUB', ip: '10.10.60.10', gateway: '10.10.60.1' });
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let i = 0; i < PER_ROW; i++) {
      ...
      const tor = inRack(r, 'c_ms250_48', 40, { name: `TOR-${row+1}-${i+1}`, ip: `10.10.10.${20 + tors.length}`,
        vlans: DC_VLANS.map(v => ({ id: v.id, name: v.name })),
      ...
      for (let s = 0; s < 6; s++) {
        ...
        const srv = inRack(r, 'server', u, { name: `SRV-${row+1}${i+1}-${s+1}`,
          nics: { 1: { ip: `10.10.20.${50 + n}`, gateway: '10.10.20.1', label: 'prod' },
                  2: { ip: `10.10.40.${50 + n}`, label: 'storage' },
                  3: { ip: `10.10.10.${100 + n}`, gateway: '10.10.10.1', label: 'oob' } } });
      ...
      // ToR trunk up to aggregation (not the core — the core has no port per rack)
      const aggPort = tors.length;                       // 1..N, agg has 48
      agg.portCfg[aggPort] = { mode: 'trunk', native: '10', tagged: DC_VLANS.map(v => v.id).join(',') };
      link(tor, 48, agg, aggPort, 0xf0883e);
  ...
  const jump = inRack(mdf, 'server', 27, { name: 'JUMP-01',
    nics: { 1: { ip: '10.10.70.10', gateway: '10.10.70.1', label: 'corp' },
            2: { ip: '10.10.10.11', gateway: '10.10.10.1', label: 'oob' } } });
  ...
  coreA.acls = parseAclText([
    ...
    ' 20 permit ip 10.10.50.0 0.0.0.255 host 10.10.50.10',
    ' 30 deny ip 10.10.50.0 0.0.0.255 any',
    ' 40 permit ip any any',
    'ip access-list extended ACCESS-ISOLATION',
    ' 10 remark door hardware talks only to the access controller',
    ' 20 permit ip 10.10.60.0 0.0.0.255 host 10.10.60.10',
    ' 30 deny ip 10.10.60.0 0.0.0.255 any',
    ' 40 permit ip any any',
    'ip access-list extended OOB-LOCKDOWN',
    ' 10 remark only the jump host reaches out-of-band management',
    ' 20 permit ip host 10.10.10.11 any',
    ' 30 permit ip host 10.10.70.10 10.10.10.0 0.0.0.255',
    ' 40 deny ip any 10.10.10.0 0.0.0.255',
